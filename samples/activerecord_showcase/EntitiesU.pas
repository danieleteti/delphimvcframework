...
// person, employee, manager
  ...
  // ======================================================================
  //  DEMO: Attribute-based validation
  //
  ...
  private
    [MVCTableField('id', [foPrimaryKey, foAutoGenerated])]
    fID: NullableInt32;
    // `code` is required, must be 1..20 chars (the column is VARCHAR(20))
    [MVCTableField('code')]
    [MVCRequired]
    [MVCMaxLength(20)]
    fCode: NullableString;
    ...
    // `city` reused as "email" for the demo because the column is varchar(200)
    [MVCTableField('city')]
    [MVCEmail]
    fEmail: string;
    [MVCTableField('rating')]
  ...
  // ======================================================================
  //  DEMO: Change Tracking ([MVCChangeTracking])
  //
  //  Mapped on the standard `customers` table. The class-level
  //  [MVCChangeTracking] attribute makes UPDATE emit only the columns
  //  whose value actually changed since the last load/save:
  //
  //      lc := TMVCActiveRecord.GetByPK<TTrackedCustomer>(42);
  //      lc.City := 'PARIS';
  //      lc.Update;            // -> UPDATE customers SET city=? WHERE id=?
  //
  //  Per-instance overhead is one CRC32 per tracked field (~4 bytes).
  //  For classes WITHOUT the attribute, no snapshot is allocated.
  //
  //  Use IsChanged / IsChanged(field) / GetChangedFields to inspect state,
  //  and UpdateIfChanged to skip SQL entirely when nothing changed (plain
  //  Update on a clean row still emits a "lock-touch" UPDATE).
  //
  //  IsChanged(FieldName) is case-insensitive (uses SameText).
  // ======================================================================
  [MVCNameCase(ncLowerCase)]
  [MVCTable('customers')]
  [MVCChangeTracking]
  TTrackedCustomer = class(TCustomEntity)
  private
    [MVCTableField('id', [foPrimaryKey, foAutoGenerated])]
    fID: NullableInt64;
    [MVCTableField('code')]
  ...
  // ======================================================================
  //  DEMO: foRefresh field option
  //
  //  Mapped on a one-off `refresh_demo` table created on the fly by the
  //  demo handler. `name` is user-controlled; `upper_name` is foRefresh
  //  and gets populated by an AFTER INSERT / AFTER UPDATE OF name trigger
  //  to UPPER(name). foRefresh tells the framework to SELECT the column
  //  back after every Insert / Update, so the in-memory instance always
  //  reflects the latest server-side value — without any manual reload
  //  on the caller's side.
  //
  //  IMPORTANT — what foRefresh is NOT:
  //    * Not a write-suppressor. The column is still fully writable; if
  //      you set it before Insert / Update, your value goes to the DB
  //      first and the trigger (if any) decides what to do with it.
  //    * Not for createdat / updatedat timestamps. Those want to be
  //      framework-driven (so they capture intent + survive lacking
  //      DEFAULTs); use [MVCAuditCreatedAt] / [MVCAuditUpdatedAt] for
  //      that — see the Audit demo.
  // ======================================================================
  [MVCNameCase(ncLowerCase)]
  [MVCTable('refresh_demo')]
  TRefreshDemo = class(TCustomEntity)
  private
    [MVCTableField('id', [foPrimaryKey, foAutoGenerated])]
    fID: NullableInt64;
    [MVCTableField('name')]
    fName: string;
    // Computed column: the DB rewrites it to UPPER(name) on every write.
    // foReadOnly keeps the framework from sending it in INSERT / UPDATE
    // (the DB owns its value); foRefresh forces a refetch through
    // RETURNING / OUTPUT so the in-memory instance mirrors the DB.
  ...
  // ======================================================================
  //  DEMO: Soft Delete ([MVCSoftDeleted])
  //
  //  Mapped on a one-off `soft_delete_demo` table created on the fly. The
  //  `deleted_at` column is NullableTDateTime — REQUIRED for timestamp
  //  mode (a plain TDateTime defaults to 1899-12-30, which would never
  //  match the auto-filter `WHERE deleted_at IS NULL`).
  //
  //  After Delete:
  //    UPDATE soft_delete_demo SET deleted_at = Now() WHERE id = ?
  //  After Restore:
  //    UPDATE soft_delete_demo SET deleted_at = NULL WHERE id = ?
  //
  //  Every SELECT against this class auto-injects `WHERE deleted_at IS
  //  NULL` unless TMVCActiveRecord.IncludeSoftDeleted(True) is in scope.
  //  HardDelete bypasses the marker and runs a physical DELETE.
  // ======================================================================
  [MVCNameCase(ncLowerCase)]
  [MVCTable('soft_delete_demo')]
  TSoftDeletableNote = class(TCustomEntity)
  private
    [MVCTableField('id', [foPrimaryKey])]
    fID: Integer;
    [MVCTableField('description')]
  ...
  //  DEMO: Audit columns
  //
  //  The four MVCAudit* attributes tell the framework to fill these fields
  //  automatically on Insert / Update:
  //    * created_at  -> set once, on Insert
  //    * updated_at  -> set on Insert, refreshed on every Update
  //    * created_by  -> current user name, captured on Insert
  //    * updated_by  -> current user name, captured on Insert AND Update
  //
  //  The "current user" is resolved from TMVCActiveRecord.GetCurrentUser,
  //  which reads the per-thread value set via TMVCActiveRecord.SetCurrentUser.
  //  In a typical web server a middleware resolves the user (e.g. from a JWT
  //  claim) and calls SetCurrentUser at the start of each request.
  //
  //  The demo uses an explicit (non-auto) PK so we don't depend on the
  //  specific auto-increment syntax of each backend.
  // ======================================================================
  [MVCTable('audit_demo')]
  TAuditedDemo = class(TMVCActiveRecord)
  private
    [MVCTableField('id', [foPrimaryKey])]
    fID: Integer;
    [MVCTableField('description')]
...
begin
  ...
  begin
    AErrors.Add('Code',
      'top-rated customers (rating=5) must have a code starting with "VIP"');
  end;
end;
