      ...
      try // only for lSelectedController
        try // global exception handler
            ...
            begin
              try
                ...
                begin
                  Log.Error('[%s] %s [PathInfo "%s"] (Custom message: "%s")',
                    [Ex.Classname, Ex.Message, GetRequestShortDescription(Context.Request.RawWebRequest), 'Cannot create controller'],
                    LOGGERPRO_TAG);
              ...
              TWebContextHack(Context).fActionQualifiedName := lRouterControllerClazzQualifiedClassName + '.' + lRouterMethodToCallName;
              lSelectedController.Engine := Self.fEngine;
              ...
              try
                lHandled := False;
                TMVCControllerHack(lSelectedController).ContentType := BuildContentType(lResponseContentMediaType, lResponseContentCharset);
