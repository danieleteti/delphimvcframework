...
Five additive features added to `TMVCActiveRecord`. All opt-in — existing
...
3. **Soft Delete** — turn `Delete` into a marker UPDATE; SELECT auto-filters
   soft-deleted rows.
4. **Validation Attributes** — declarative field-level validators evaluated
   before every `Insert` / `Update`.
5. **Audit Columns** — automatic population of created/updated timestamps and
   user names without a single line of override code.
...
> Note: plain `TGUID` PKs are not supported by the framework's PK-type
> check (`TryGetPKValue` rejects any `tkRecord` that is not a recognized
> `Nullable*`). Use `NullableTGUID` if you need a GUID-shaped primary key.
...
Validation logic scattered across `OnValidation` override methods is
hard to discover and easy to forget. Declarative attributes colocate
the constraint with the field definition, are reusable across entities,
and produce structured error messages that the framework serialises to
HTTP 422 automatically.

### Enable

Attach one or more validator attributes to a field. The framework
evaluates them in `Insert` and `Update` before any SQL is emitted.
...
| Attribute | Applies to | Rejects when... |
|---|---|---|
| `[MVCRequired]` | string, Nullable | value is empty / `HasValue = False` |
| `[MVCNotEmpty]` | string | empty or whitespace-only (accepts `null` Nullable) |
| `[MVCMinLength(N)]` | string, Nullable | `Length < N` |
| `[MVCMaxLength(N)]` | string, Nullable | `Length > N` |
| `[MVCLength(N)]` | string, Nullable | `Length <> N` |
| `[MVCRange(min, max)]` | Integer, Int64, Nullable variants | value < min or value > max |
| `[MVCEmail]` | string, Nullable | not a valid e-mail address |
| `[MVCPattern(regex)]` | string, Nullable | string does not match the regex |
| `[MVCIn('a,b,c')]` | string, Nullable | value not in the comma-separated allowlist |

All validators handle **Nullable types** transparently: a Nullable field
whose `HasValue` is `False` is treated as a missing value, consistent
with how `[MVCRequired]` expects a value to be present.

### How errors are reported

When one or more validators fail, the framework collects **all** field
errors in a single pass and raises `EMVCStorageValidationException`
(inherits from `EMVCValidationException`) with a populated
...
try
  ...
  begin
    // E.ValidationErrors is a TDictionary<string, string>:
...
The key is the **field name** (leading `F` or `f` stripped from the
RTTI field identifier). Validators on `foAutoGenerated`, `foReadOnly`,
`foDoNotInsert`, and PK fields are skipped on `Insert` because the
framework has not yet assigned a value to those fields at validation
time.

### Combining with OnStorageValidate

`OnStorageValidate` fires **after** all attribute validators. Use it for
cross-field or business-rule constraints that cannot be expressed with a
single attribute:

```pascal
procedure TUser.OnStorageValidate(const AErrors: PMVCValidationErrors;
  const EntityAction: TMVCEntityAction);
begin
  if (FEndDate > 0) and (FStartDate > FEndDate) then
    AErrors.Add('StartDate', 'StartDate must be before EndDate');
end;
```

All errors — attribute-based and `OnStorageValidate`-based — are
aggregated into the same exception, so callers receive the full picture
in one shot.

### Custom error messages

Every validator accepts an optional trailing `AMessage: string`
parameter. When omitted, the framework uses a built-in default:

```pascal
[MVCRequired('Email address is mandatory')]
[MVCMaxLength(255, 'Email cannot exceed 255 characters')]
FEmail: string;
```
...
Tracking who created or last modified a row, and when, is a
cross-cutting concern present in most business applications. Without
framework support it means overriding `OnBeforeInsert` and
`OnBeforeUpdate` in every entity, duplicating the same timestamp /
current-user boilerplate.

Audit attributes make this declarative and zero-code per entity.
...
TInvoice = class(TMVCActiveRecord)
  ...
  [MVCTableField('amount')]
  FAmount: Currency;

  // Timestamps — set once on Insert; UpdatedAt refreshed on every Update
  [MVCTableField('created_at')]
  [MVCAuditCreatedAt]
  FCreatedAt: TDateTime;            // or NullableTDateTime

  [MVCTableField('updated_at')]
  [MVCAuditUpdatedAt]
  FUpdatedAt: TDateTime;            // or NullableTDateTime

  // User tracking — set from the per-thread current-user value
  [MVCTableField('created_by')]
  [MVCAuditCreatedBy]
  FCreatedBy: string;               // or NullableString

  [MVCTableField('updated_by')]
  [MVCAuditUpdatedBy]
  FUpdatedBy: string;               // or NullableString
end;
```

You can use any subset of the four attributes; they are independent.

### Supported field types

| Attribute | Accepted Delphi types |
|---|---|
| `[MVCAuditCreatedAt]` / `[MVCAuditUpdatedAt]` | `TDateTime`, `NullableTDateTime`, `TDate`, `NullableTDate` |
| `[MVCAuditCreatedBy]` / `[MVCAuditUpdatedBy]` | `string`, `NullableString` |

### What the framework does automatically

| Attribute | On `Insert` | On `Update` |
|---|---|---|
| `[MVCAuditCreatedAt]` | Set to `Now` | **Unchanged** |
| `[MVCAuditUpdatedAt]` | Set to same `Now` as `CreatedAt` | Refreshed to current `Now` |
| `[MVCAuditCreatedBy]` | Set to `GetCurrentUser` (if non-empty) | **Unchanged** |
| `[MVCAuditUpdatedBy]` | Set to `GetCurrentUser` (if non-empty) | Refreshed to current `GetCurrentUser` |

Audit fields are populated **before** `OnBeforeInsert` / `OnBeforeUpdate`
fire, so hooks see the already-set values.

### Setting the current user (per thread)

```pascal
// In your auth middleware or request begin handler:
TMVCActiveRecord.SetCurrentUser(lJWTClaims.Subject);

// At the end of the request (or in a pooled thread's cleanup):
TMVCActiveRecord.SetCurrentUser('');
```

The current user is stored in a **threadvar** — each thread has its own
value and cannot affect others. If `SetCurrentUser` was never called on
the current thread, `GetCurrentUser` returns `''` and the `*By` fields
are left unchanged (not overwritten with an empty string).

### Example flow

```pascal
// Request handler — user 'alice' creates an invoice
...
try
  lInv.Amount := 1500.00;
  lInv.Insert;
  // lInv.FCreatedAt  = Now  (set by framework)
  // lInv.FUpdatedAt  = Now  (same as CreatedAt)
  // lInv.FCreatedBy  = 'alice'
  // lInv.FUpdatedBy  = 'alice'
finally
  lInv.Free;
end;

// Later — user 'bob' edits the same invoice
TMVCActiveRecord.SetCurrentUser('bob');
lInv := TMVCActiveRecord.GetByPK<TInvoice>(lID);
try
  lInv.Amount := 2000.00;
  lInv.Update;
  // lInv.FCreatedAt  = original Insert timestamp (UNCHANGED)
  // lInv.FUpdatedAt  = Now  (refreshed)
  // lInv.FCreatedBy  = 'alice' (UNCHANGED)
  // lInv.FUpdatedBy  = 'bob'  (refreshed)
finally
  lInv.Free;
end;
```
...
| Soft Delete × Audit Columns | `MVCAuditUpdatedAt` / `MVCAuditUpdatedBy` fire on soft-delete and on `Restore` of a single instance (both go through the UPDATE path that calls `FillAuditFields`). `MVCAuditCreatedAt` / `MVCAuditCreatedBy` are never touched on soft-delete or Restore. |
| Soft Delete × Validation | Attribute validators and `OnStorageValidate` do **not** fire on soft-delete or Restore. These are marker operations, not content changes. |
...
| Audit Columns × `[MVCChangeTracking]` | Audit fields (UpdatedAt / UpdatedBy) are set by `FillAuditFields` **before** the dirty-aware SET clause is built, so they are always included in the UPDATE regardless of whether the user changed them. This ensures a fresh timestamp and user name on every save. |
| Audit Columns × Validation | Audit fields are populated by `FillAuditFields` **after** validation passes. A failed validation therefore leaves audit fields at their pre-call values. |
| Validation × `OnStorageValidate` | Attribute validators run first; `OnStorageValidate` runs second. All errors are accumulated in the same pass and reported together in `EMVCStorageValidationException`. |
...
// ── Field-level attributes — validators ──────────────────────────────────
// All constructors accept an optional trailing AMessage: string = ''.
MVCRequired    = class(TMVCValidatorBase);
MVCNotEmpty    = class(TMVCValidatorBase);
MVCMinLength   = class(TMVCValidatorBase);   // Create(AMinLength: Integer; ...)
MVCMaxLength   = class(TMVCValidatorBase);   // Create(AMaxLength: Integer; ...)
MVCLength      = class(TMVCValidatorBase);   // Create(AExactLength: Integer; ...)
MVCRange       = class(TMVCValidatorBase);   // Create(AMin, AMax: Int64; ...)
MVCEmail       = class(TMVCValidatorBase);
MVCPattern     = class(TMVCValidatorBase);   // Create(ARegex: string; ...)
MVCIn          = class(TMVCValidatorBase);   // Create(AAllowedValues: string; ...)

// ── Field-level attributes — audit ───────────────────────────────────────
MVCAuditCreatedAtAttribute = class(MVCActiveRecordCustomAttribute);
MVCAuditUpdatedAtAttribute = class(MVCActiveRecordCustomAttribute);
MVCAuditCreatedByAttribute = class(MVCActiveRecordCustomAttribute);
...
// Validation (override to add cross-field rules)
procedure OnStorageValidate(const AErrors: PMVCValidationErrors;
  const EntityAction: TMVCEntityAction); virtual;
...
// Audit — current user (threadvar; safe in multi-threaded servers)
class procedure SetCurrentUser(const AUserName: string);
class function  GetCurrentUser: string;

// ── Bulk operations on TMVCActiveRecordHelper ─────────────────────────────
class function DeleteRQL<T>(const RQL: string = ''): Int64;    // smart soft/hard
class function HardDeleteRQL<T>(const RQL: string = ''): Int64; // always physical
class function RestoreRQL<T>(const RQL: string = ''): Int64;    // bulk un-delete

// ── Exceptions ────────────────────────────────────────────────────────────
// EMVCStorageValidationException inherits EMVCValidationException.
// ValidationErrors: TDictionary<string, string>  (FieldName -> Message)
```
