...
unit MVCFramework.Filters.ActiveRecord;
...
type
  ...
  public
    constructor Create(const DefaultConnectionDefName: string); overload; virtual;
    constructor Create(const DefaultConnectionDefName: string;
      const ConnectionDefFileName: string { = 'FDConnectionDefs.ini' } ); overload; virtual;
    constructor Create(const DefaultConnectionDefName: string; const AdditionalARConnectionNames: TArray<String>;
...
constructor TMVCActiveRecordProtocolFilter.Create(const DefaultConnectionDefName: string;
...
constructor TMVCActiveRecordProtocolFilter.Create(const DefaultConnectionDefName: string;
  const AdditionalARConnectionNames, AdditionalConnectionDefNames: TArray<String>; const ConnectionDefFileName: string);
begin
  inherited Create;
...
procedure TMVCActiveRecordProtocolFilter.DoFilter(Context: TWebContext);
var
  I: Integer;
begin
  try
...
procedure TMVCActiveRecordProtocolFilter.EnsureConnection;
var
  I: Integer;
begin
      ...
      begin
        ...
        begin
          raise EMVCConfigException.Create
            ('AdditionalARConnectionNames must have the same length of AdditionalConnectionDefNames');
        end;
        for I := 0 to fAdditionalConnectionDefNamesCount - 1 do
        begin
