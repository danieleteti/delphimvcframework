...
type
  ...
  protected
    class function NeedsToBeExtended(const JWTValue: TJWT): Boolean;
    class procedure ExtendExpirationTime(const JWTValue: TJWT);
    ...
    type
      ...
      public
        constructor Create(
          ...
          AAuthorizationHeaderName: string;
          AAuthorizationAccessToken: string;
          ASecret: string;
          AClaimsToChecks: TJWTCheckableClaims;
...
constructor TMVCJWTBlackListProtocolFilter.Create(
  OnAcceptToken: TMVCOnAcceptTokenProc;
  OnNewJWTToBlackList: TMVCOnNewJWTToBlackList;
  BlackListRequestURLSegment: string = '/logout');
begin
...
procedure TMVCJWTBlackListProtocolFilter.DoFilter(Context: TWebContext);
var
  lAuthHeader: string;
  lAuthToken: string;
  lAccepted: Boolean;
...
begin
  lHandled := True;
  lAuthToken := '';
  lAuthHeader := Context.Request.Headers[TMVCJWTDefaults.AUTHORIZATION_HEADER];
  ...
  if SameText(Context.Request.PathInfo, fBlackListRequestURLSegment) then
  begin
    ...
    fOnNewJWTToBlackList(Context, lAuthToken);
  ...
  begin
    ...
    begin
      lAccepted := True;
      fOnAcceptToken(Context, lAuthToken, lAccepted);
...
procedure TMVCJWTBlackListProtocolFilter.OnAfterControllerAction(
      AContext: TWebContext;
      const AControllerQualifiedClassName: string; const AActionName: string;
      const AHandled: Boolean);
begin
...
procedure TMVCJWTBlackListProtocolFilter.OnAfterRouting(AContext: TWebContext; const AHandled: Boolean);
begin
  // Implement as needed
end;

procedure TMVCJWTBlackListProtocolFilter.OnBeforeControllerAction(
...
procedure TMVCJWTBlackListProtocolFilter.OnBeforeRouting(AContext: TWebContext; var AHandled: Boolean);
var
  lAuthHeader: string;
  lAuthToken: string;
  lAccepted: Boolean;
...
{ TMVCJWTProtocolFilter }

constructor TMVCJWTProtocolFilter.Create(
  AAuthenticationHandler: IMVCAuthenticationHandler;
  AConfigClaims: TJWTClaimsSetup; ASecret, ALoginURLSegment: string;
  AClaimsToCheck: TJWTCheckableClaims; ALeewaySeconds: Cardinal;
...
procedure TMVCJWTProtocolFilter.DoFilter(Context: TWebContext);
var
  LUsername: string;
  LPassword: string;
  ...
  lJObj: TJsonObject;
begin
  if SameText(Context.Request.PathInfo, FLoginURLSegment) then
  begin
    LBasicAuthHeader := Context.Request.Headers[FAuthorizationHeaderName];
    if LBasicAuthHeader.IsEmpty then
    begin
      // read from headers
      LUsername := TNetEncoding.URL.Decode(Context.Request.Headers[FUserNameHeaderName]);
      LPassword := TNetEncoding.URL.Decode(Context.Request.Headers[FPasswordHeaderName]);

      // read from content
      if LUsername.IsEmpty and not SameText(Context.Request.ContentMediaType, TMVCMediaType.APPLICATION_JSON) then
      begin
        LUsername := Context.Request.ContentParam(FUserNameHeaderName);
        LPassword := Context.Request.ContentParam(FPasswordHeaderName);
      end;

      // read from json content
      if LUsername.IsEmpty then
      begin
        lJObj := StrToJSONObject(Context.Request.Body, False);
        try
    ...
    try
      ...
      try
        ...
        begin
          FAuthenticationHandler.OnAuthentication(Context, LUsername, LPassword, LRolesList, LIsValid, LSessionData);
        ...
        try
          ...
          LJWTValue.Data := Context.Request;
          ...
          Context.LoggedUser.Roles.AddRange(LRolesList);
          Context.LoggedUser.UserName := LJWTValue.CustomClaims['username'];
          Context.LoggedUser.LoggedSince := LJWTValue.Claims.IssuedAt;
          Context.LoggedUser.Realm := LJWTValue.Claims.Subject;

          if LSessionData.Count > 0 then
          begin
            Context.LoggedUser.CustomData := TMVCCustomData.Create;
            for LCustomPair in LSessionData do
            begin
              Context.LoggedUser.CustomData.AddOrSetValue(LCustomPair.Key, LCustomPair.Value);
...
class procedure TMVCJWTProtocolFilter.ExtendExpirationTime(const JWTValue: TJWT);
begin
...
procedure TMVCJWTProtocolFilter.InternalRender(AJSONOb: TJDOJsonObject;
...
class function TMVCJWTProtocolFilter.NeedsToBeExtended(const JWTValue: TJWT): Boolean;
var
  lWillExpireIn: Int64;
begin
...
procedure TMVCJWTProtocolFilter.OnAfterRegistration(Engine: TMVCEngine);
begin
  inherited;
  Engine.UseFilter(TMVCJWTControllerFilter.Create(
    FAuthenticationHandler,
    FAuthorizationHeaderName,
    FAuthorizationAccessToken,
    FSecret,
    FClaimsToChecks,
    FLeewaySeconds
  ));
...
{ TMVCJWTProtocolFilter.TMVCJWTControllerFilter }

constructor TMVCJWTProtocolFilter.TMVCJWTControllerFilter.Create(
          AAuthenticationHandler: IMVCAuthenticationHandler;
          AAuthorizationHeaderName: string;
          AAuthorizationAccessToken: string;
          ASecret: string;
...
begin
  ...
  FAuthorizationAccessToken := AAuthorizationAccessToken;
  FSecret := ASecret;
  FClaimsToChecks := AClaimsToChecks;
  FLeewaySeconds := ALeewaySeconds;
end;

procedure TMVCJWTProtocolFilter.TMVCJWTControllerFilter.DoFilter(
...
begin
  ...
  if Assigned(FAuthenticationHandler) then
  begin
    FAuthenticationHandler.OnRequest(Context, Router.ControllerClazz.QualifiedClassName, Router.ActionMethod.Name, AuthRequired);
    ...
    begin
      ...
      // retrieve the token from the "authentication Bearer" header
      AuthHeader := Context.Request.Headers[FAuthorizationHeaderName];
      if AuthHeader.IsEmpty then
        // retrieve the token from the "access_token" query param
        AuthHeader := Context.Request.Params[FAuthorizationAccessToken];
      ...
      begin
        ...
        try
          ...
          begin
            ...
            Context.LoggedUser.UserName := JWTValue.CustomClaims['username'];
            Context.LoggedUser.Roles.AddRange(JWTValue.CustomClaims['roles'].Split([',']));
            Context.LoggedUser.LoggedSince := JWTValue.Claims.IssuedAt;
            Context.LoggedUser.CustomData := JWTValue.CustomClaims.AsCustomData;
          end;
        finally
          JWTValue.Free;
        end;
  ...
  try
    ...
    begin
      // retrieve the token from the "access_token" query param
      AuthAccessToken := Context.Request.Params[FAuthorizationAccessToken];
    ...
    if Assigned(FAuthenticationHandler) then
    begin
      FAuthenticationHandler.OnAuthorization(Context, Context.LoggedUser.Roles, Router.ControllerClazz.QualifiedClassName,
        Router.ActionMethod.Name, IsAuthorized);
    ...
    begin
      ...
      begin
        ...
        Context.Response.SetCustomHeader(FAuthorizationHeaderName, 'Bearer ' + JWTValue.GetToken);
      end;
    end;
