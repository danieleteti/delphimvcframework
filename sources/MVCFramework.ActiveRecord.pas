...
{$ENDIF}
  ...
  private
    ...
    /// which is the only shape PrimaryKeyIsAutogenerated can describe.</summary>
...
begin
  { PrimaryKeyIsAutogenerated describes ONE key column, so on anything else it
    has no honest answer and must not invent one. Answering from column [0]
    would report False for a composite key whose generated column sits
    elsewhere - a quiet wrong answer that only turns into a visible bug later,
    at Insert time. Ask fAutoGenPKIndex when you need the composite question. }
  if fTableMap = nil then
    raise EMVCActiveRecord.Create('Entity has no table mapping');
  if not fTableMap.HasPK then
    raise EMVCActiveRecord.Create(
      'Entity has no primary key; PrimaryKeyIsAutogenerated describes a primary key column');
  if fTableMap.IsCompositePK then
    raise EMVCActiveRecord.Create(
      'Entity has a composite primary key; PrimaryKeyIsAutogenerated describes a single key column, ' +
      'use the foAutoGenerated option on the specific column instead');
end;
