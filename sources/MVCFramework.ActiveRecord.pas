...
begin
  ...
  lProviderName := aConnection.ProviderName;
  if SameText(lProviderName, 'Oracle') then
    Result := TMVCActiveRecordBackEnd.Oracle
  else if SameText(lProviderName, 'SQLServer') then
    Result := TMVCActiveRecordBackEnd.MSSql
  else if SameText(lProviderName, 'Access') then
    Result := TMVCActiveRecordBackEnd.MSAccess
  else if SameText(lProviderName, 'MySQL') then
    Result := TMVCActiveRecordBackEnd.MySQL
  else if SameText(lProviderName, 'DB2') then
    Result := TMVCActiveRecordBackEnd.DB2
  else if SameText(lProviderName, 'ASE') then
    Result := TMVCActiveRecordBackEnd.SQLAnywhere
  else if SameText(lProviderName, 'Advantage') then
    Result := TMVCActiveRecordBackEnd.Advantage
  else if SameText(lProviderName, 'Interbase') then
    Result := TMVCActiveRecordBackEnd.Interbase
  else if SameText(lProviderName, 'Firebird') then
    Result := TMVCActiveRecordBackEnd.Firebird
  else if SameText(lProviderName, 'SQLite') then
    Result := TMVCActiveRecordBackEnd.SQLite
  else if SameText(lProviderName, 'PostgreSQL') then
    Result := TMVCActiveRecordBackEnd.PostgreSQL
  else if SameText(lProviderName, 'NexusDB') then
    Result := TMVCActiveRecordBackEnd.NexusDB
  else if SameText(lProviderName, 'DataSnap') then
    Result := TMVCActiveRecordBackEnd.DataSnap
  else if SameText(lProviderName, 'Informix') then
    Result := TMVCActiveRecordBackEnd.Informix
  else if SameText(lProviderName, 'Teradata') then
    Result := TMVCActiveRecordBackEnd.Teradata
  else if SameText(lProviderName, 'MongoDB') then
    Result := TMVCActiveRecordBackEnd.MongoDB
  else
    Result := TMVCActiveRecordBackEnd.Other;
end;
...
begin
  ...
  begin
    aConnection.Transaction := TUniTransaction.Create(aConnection);
  end;
  aConnection.Transaction.IsolationLevel := TTransactionIsolation.tiReadCommitted;
...
class function TMVCActiveRecord.ExecQuery(const SQL: string; const Values: array of Variant;
  const Connection: TUniConnection; const Unidirectional: Boolean;
...
begin
  lQry := CreateQuery(True, True);
  try
    lQry.Connection := CurrentConnection;
    lQry.SQL.Text := SQL;
    lQry.Params.CreateParam(ftInteger, 'p1', ptInput).AsInteger := 1;
    lQry.Open(SQL, Params);
...
{$ENDIF}
    ...
    tkEnumeration:
      begin
        ...
        begin
          if SameText(aParam.Name, 'int') then
          begin
...
class function TMVCActiveRecordHelper.Select(const aClass: TMVCActiveRecordClass; const SQL: string;
  const Params: array of Variant; const Connection: TUniConnection): TMVCActiveRecordList;
begin
...
class function TMVCActiveRecordHelper.Where<T>(const SQLWhere: string;
  const Params: array of Variant; const ParamTypes: array of TFieldType): TObjectList<T>;
begin
  Result := TObjectList<T>.Create(True);
  try
...
class function TMVCActiveRecordHelper.Where(const aClass: TMVCActiveRecordClass;
  ...
  const Connection: TUniConnection; const OutList: TMVCActiveRecordList): UInt32;
var
  lAR: TMVCActiveRecord;
begin
  lAR := aClass.Create;
