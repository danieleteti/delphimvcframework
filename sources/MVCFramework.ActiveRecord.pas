...
{$ENDIF}
  ...
  TMVCActiveRecordFieldOption = (
    ...
    foDoNotUpdate,
    /// <summary>
    /// After Insert/Update, refetch this field's value from the database
    /// (DB DEFAULT, computed column, trigger). Implemented natively via
    /// RETURNING where the engine supports it; SQL Server, which has no BEFORE
    /// triggers, selects the row again by key in the same batch; falls back
    /// to a post-execute SELECT on MySQL.
    /// </summary>
    foRefresh
  );
  ...
  IMVCEntityProcessor = interface
    ...
    procedure GetEntities(const Context: TWebContext; const Renderer: TMVCRenderer; const entityname: string;
      var Handled: Boolean);
    // id is a string (was Integer up to 3.4): it carries the raw ($id) URL segment,
    // so it can address string/GUID keys and composite keys ("k1;k2").
    procedure GetEntity(const Context: TWebContext; const Renderer: TMVCRenderer; const entityname: string;
      const id: string; var Handled: Boolean);
  ...
  /// <summary>
  /// Marks a TDateTime field to be auto-filled on Insert with the current timestamp.
  /// The same value is also assigned to any MVCAuditUpdatedAt field on Insert,
  /// so ORDER BY updated_at is stable without NULLs.
  /// </summary>
  MVCAuditCreatedAtAttribute = class(MVCActiveRecordCustomAttribute);

  /// <summary>
  /// Marks a TDateTime field to be auto-filled on Insert and refreshed on Update.
  /// </summary>
  MVCAuditUpdatedAtAttribute = class(MVCActiveRecordCustomAttribute);

  /// <summary>
  /// Marks a string/NullableString field to be auto-filled on Insert with
  /// TMVCActiveRecord.GetCurrentUser. Remains unchanged on Update.
  /// </summary>
  MVCAuditCreatedByAttribute = class(MVCActiveRecordCustomAttribute);

  /// <summary>
  /// Marks a string/NullableString field to be auto-filled on Insert and refreshed
  /// on Update with TMVCActiveRecord.GetCurrentUser.
  /// </summary>
  ...
  ///   - TDateTime / NullableTDateTime → timestamp mode (delete = Now(),
  ///     restore = NULL).
  ///   - Boolean → flag mode (delete = TRUE, restore = FALSE).
  /// Any other type → exception at TableMap build time. At most one
  /// [MVCSoftDeleted] field per class.</summary>
  ...
  /// </summary>
  TMVCFieldValidator = record
    RttiField: TRttiField;
    Validators: TArray<TMVCValidatorBase>;
    SkipOnInsert: Boolean;  // true for foAutoGenerated/foReadOnly/foDoNotInsert/PK
  end;

  TMVCSoftDeleteMode = (sdmNone, sdmTimestamp, sdmFlag);

  /// <summary>
  /// One primary-key column. A table map holds an array of these; a single-column
  /// PK is just the degenerate case with one element. The legacy scalar fPrimaryKey*
  /// fields on TMVCTableMap mirror element [0] for backward compatibility.
  /// </summary>
  TMVCPKInfo = record
    RTTIField: TRTTIField;
    FieldName: string;
    Options: TMVCActiveRecordFieldOptions;
    FieldType: TFieldType; // ftLargeInt/ftInteger/ftString/ftGuid
    SequenceName: string;
    InInsert: Boolean;     // [foAutoGenerated,foReadOnly,foDoNotInsert] * Options = []
  end;
  ...
  public
    ...
    fPrimaryKeys: TArray<TMVCPKInfo>; // all PK columns; scalar fPrimaryKey* mirror [0]
    fAutoGenPKIndex: Integer;         // index in fPrimaryKeys of the foAutoGenerated PK column, or -1
    fNamedSQLQueries: TArray<TSQLQueryWithName>;
    fNamedRQLQueries: TArray<TRQLQueryWithName>;
    // Validation/audit metadata (built once per class in InitTableInfo)
    fHasValidators: Boolean;
    fFieldValidators: TArray<TMVCFieldValidator>;
    fAuditCreatedAtField: TRttiField;
    fAuditUpdatedAtField: TRttiField;
    fAuditCreatedByField: TRttiField;
    fAuditUpdatedByField: TRttiField;
  public
    constructor Create;
    destructor Destroy; override;
    function VersionValueAsInt64For(AR: TMVCActiveRecord): Int64; //inline;
    function IsCompositePK: Boolean; // Length(fPrimaryKeys) > 1
    function HasPK: Boolean;         // Length(fPrimaryKeys) > 0
    function AnyPKInInsert: Boolean;  // at least one PK column is emitted in INSERT
    function TryGetPKOptions(const AField: TRTTIField;
      out AOptions: TMVCActiveRecordFieldOptions): Boolean;
    ...
    function IsSoftDeleteEnabled: Boolean;
  end;
  ...
  private
    ...
    procedure FillAuditFields(const EntityAction: TMVCEntityAction);
    procedure CollectFieldValidatorErrors(const AErrors: PMVCValidationErrors;
      const EntityAction: TMVCEntityAction);
    procedure SetAuditDateTimeField(const AField: TRttiField; const AValue: TDateTime);
    procedure SetAuditStringField(const AField: TRttiField; const AValue: string);
    procedure SetPKField(const APKField: TRTTIField; const aValue: TValue);
    function VarRecToTValueForPK(const APKField: TRTTIField; const AItem: TVarRec;
      const AFieldType: TFieldType): TValue;
    function VarRecToVariantForPK(const AItem: TVarRec; const AFieldType: TFieldType): Variant;
    function DoLoadByPKSelect(const aParams: array of Variant;
      const aParamTypes: array of TFieldType): Boolean;
    function ReloadByCurrentPK: Boolean;
    function PKColumnValueAsString(const AIndex: Integer): string;
    function GetPrimaryKeyIsAutogenerated: Boolean;
    procedure SetPrimaryKeyIsAutogenerated(const Value: Boolean);
    /// <summary>Raises unless the entity has exactly one primary-key column,
    /// which is the only shape the PrimaryKeyIsAutogenerated setter can act
    /// on. The getter answers for every shape and does not use this.</summary>
    procedure CheckSinglePKForAutogenFlag;
  ...
  protected
    ...
    /// <summary>RefreshAutoGenerated / RefreshFields: the statement is this
    /// entity's own INSERT / UPDATE and its generated key / foRefresh columns
    /// must be read back. Everything else (deletes, bulk statements) passes
    /// neither and is simply executed.</summary>
    function ExecNonQuery(const SQL: string; RefreshAutoGenerated: Boolean = false;
      RefreshFields: Boolean = false): Int64; overload;
    ...
    /// <summary>
    /// Storage-side validation hook. Override to collect cross-field /
    /// business errors that depend on the save context. The framework calls
    /// it ONLY from Insert (EntityAction = eaCreate) and Update
    /// (EntityAction = eaUpdate); errors added to AErrors are merged with
    /// the attribute validators and raised together as
    /// EMVCStorageValidationException. NEVER called during HTTP-body
    /// deserialization — for boundary / cross-field rules override
    /// TMVCValidatable.OnValidate (inherited) and declare attribute
    /// validators on public properties, not on private fields.
    /// </summary>
  ...
  public
    ...
    procedure Store;
    /// <summary>
    ///   Re-reads this instance from the database by its current primary key,
    ///   discarding any unsaved local edits. Returns True when a row was found.
    ///   When no row matches it raises EMVCActiveRecordNotFound by default (a
    ///   silent no-op here would leave a stale copy in place); pass
    ///   RaiseExceptionIfNotFound = False to get False instead. Behaves like Load.
    /// </summary>
    function Refresh(const RaiseExceptionIfNotFound: Boolean = True): Boolean; virtual;
    /// <summary>
    ///   Populates this instance from the database using the primary-key value(s)
    ///   currently set on its fields (assigned by name through the typed
    ///   properties). Order-independent: the key is set by property name, not by
    ///   position. Reads naturally for a first load: set the key properties, then
    ///   call Load. Returns True when a row was found. When no row matches it
    ///   raises EMVCActiveRecordNotFound by default; pass RaiseExceptionIfNotFound
    ///   = False to get False instead, leaving the instance untouched.
    /// </summary>
    function Load(const RaiseExceptionIfNotFound: Boolean = True): Boolean;
    function CheckAction(const aEntityAction: TMVCEntityAction;
      const aRaiseException: Boolean = True): Boolean;
    procedure Insert;
    /// <summary>
    /// Runs attribute-based validators plus the optional OnStorageValidate hook.
    /// Raises EMVCStorageValidationException if any error is produced; the
    /// field-level messages are exposed via the ValidationErrors dictionary.
    /// Validators declared on foAutoGenerated/foReadOnly/foDoNotInsert fields
    /// and on the primary key are skipped when EntityAction = eaCreate.
    /// </summary>
    procedure Validate(const EntityAction: TMVCEntityAction = eaUpdate);
    ...
    /// <summary>Loads a row identified by a composite primary key. Pass one value
    /// per PK column, in declaration order. Works for single-PK tables too.</summary>
    function LoadByPKs(const aValues: array of const): Boolean; overload; virtual;
    /// <summary>Same as LoadByPKs, values supplied as strings (e.g. from a URL);
    /// each is coerced by the corresponding PK column type.</summary>
    function LoadByPKs(const aValues: TArray<string>): Boolean; overload; virtual;
    function HasCompositePK: Boolean;
    /// <summary>The primary-key value(s) rendered for a URL path segment: the
    /// scalar value for a single key, or a JSON array (typed) for a composite key.</summary>
    function PKAsURLSegment: string;
    /// <summary>The primary key rendered as a JSON array of typed values: numbers
    /// for integer columns, JSON strings (properly escaped) for string/GUID columns.
    /// e.g. [1,"ACME;01",true-not-used]. Used to address composite keys in a URL.</summary>
    function PKAsJSONArray: string;
    ...
    /// <summary>Returns one TValue per PK column, in declaration order.</summary>
    function GetPKs: TArray<TValue>;
    /// <summary>Sets every PK column from the supplied values (declaration order).</summary>
    procedure SetPKs(const aValues: array of const); overload;
    /// <summary>Sets every PK column from string values (e.g. from a URL).</summary>
    procedure SetPKs(const aValues: TArray<string>); overload;
    ...
    { MVCDoNotDeserialize matters as much as MVCDoNotSerialize here, and for a
      sharper reason: the deserializer skips properties by the DESERIALIZE
      attribute, so with only the other one every incoming JSON body was writing
      this flag back, and a body that carried "PrimaryKeyIsAutogenerated":true
      would have switched it - on the table map, which is built once per class
      and shared by every instance of it. It describes the mapping, not the row:
      it has no business travelling on the wire in either direction. }
    [MVCDoNotSerialize]
    [MVCDoNotDeserialize]
    ...
    /// <summary>
    /// Sets the current user name for the calling thread (used by
    /// MVCAuditCreatedBy / MVCAuditUpdatedBy audit columns). Stored in a
    /// threadvar: each thread has its own value and cannot affect other
    /// threads. Call it at the start of each request/handler with the
    /// user you already resolved (e.g. from a JWT claim) and, if your
    /// threads are pooled, reset it to '' at the end.
    /// </summary>
    class procedure SetCurrentUser(const AUserName: string);

    /// <summary>
    /// Returns the current user name for the calling thread, or an empty
  ...
  TMVCActiveRecordHelper = class helper for TMVCActiveRecord
    ...
    class function GetByPK<T: TMVCActiveRecord, constructor>(const aValue: TGuid;
      const RaiseExceptionIfNotFound: Boolean = True): T; overload;
    /// <summary>Loads T by a (possibly composite) primary key. Pass one value per
    /// PK column in declaration order. Works for single-PK entities too.</summary>
    class function GetByPKs<T: TMVCActiveRecord, constructor>(const aValues: array of const;
...
{$ENDIF}
    ...
    { SelectUnidirectionalDataSet — alias of SelectDataSet(..., Unidirectional=True).
      Returns a forward-only TFDQuery (Unidirectional + fmOnDemand + ReadOnly) that
      fetches rows in RowsetSize-sized windows instead of materializing the whole
      resultset in the client-side buffer at Open.

      Use whenever the caller walks the rows once without needing random access:
      streaming JSON serialization from a functional action, ETL / data migration,
      batch iteration, export to any sink (CSV, XML, custom). The dataset does NOT
      support bookmarks, Prior, Locate or scrolling — those operations raise. }
    class function SelectUnidirectionalDataSet(const SQL: string;
      const Params: array of Variant;
      const DirectExecute: Boolean = False): TDataSet; overload;
    class function SelectUnidirectionalDataSet(const SQL: string;
      const Params: array of Variant;
  ...
  protected
    ...
    function GetMapping: TMVCFieldsMapping;
    function TableFieldsDelimited(const Map: TFieldsMap; const PKFieldName: string; const Delimiter: string): string; overload;
    /// <summary>Composite-aware select list: prepends ALL primary-key columns
    /// (in declaration order) to the selectable fMap columns. For a single-PK
    /// table it is byte-identical to the PKFieldName overload.</summary>
    function TableFieldsDelimited(const Map: TFieldsMap; const TableMap: TMVCTableMap; const Delimiter: string): string; overload;
    /// <summary>Builds "col1 &lt;sep&gt; :col1 and col2 &lt;sep&gt; :col2" over every PK
    /// column. AEqualsSep is the field/param separator ('= :' or '=:'), kept
    /// per call-site so single-PK output stays byte-identical.</summary>
    function BuildPKWhereConjunction(const TableMap: TMVCTableMap; const AEqualsSep: string): string;
    /// <summary>Comma-terminated list of SQL-quoted PK column names to include in
    /// an INSERT column list (those with InInsert=true). Empty when none.
    /// Single-PK, in-insert: byte-identical to the old inline "col," emission.</summary>
    function PKInsertColumns(const TableMap: TMVCTableMap): string;
    /// <summary>Comma-terminated list of ":param" placeholders matching
    /// PKInsertColumns, in the same order.</summary>
    function PKInsertParams(const TableMap: TMVCTableMap): string;
    /// <summary>SQL-quoted column name of the (single) auto-generated PK column,
    /// wherever it sits in a composite key. Empty string when there is none.</summary>
    function AutoGenPKFieldName(const TableMap: TMVCTableMap): string;
    /// <summary>Returns a comma-separated, SQL-quoted list of columns the
  ...
  public
    ...
    /// <summary>A query appended to an INSERT/UPDATE/DELETE that reads nothing
    /// back, returning the statement's row count as its only column. Empty
    /// (the default) means the driver's count is used.</summary>
    function GetRowsAffectedSQL: string; virtual;
    // end-capabilities
    ...
    function CreateSelectSQL(const TableName: string; const Map: TFieldsMap; const PKFieldName: string;
      const PKOptions: TMVCActiveRecordFieldOptions): string; overload; virtual;
    /// <summary>Composite-aware SELECT (column list includes all PK columns).
    /// Byte-identical to the PKFieldName overload for single-PK tables.</summary>
    function CreateSelectSQL(const TableMap: TMVCTableMap): string; overload; virtual;
    ...
    function CreateSelectByPKSQL(const TableName: string; const Map: TFieldsMap; const PKFieldName: string;
      const PKOptions: TMVCActiveRecordFieldOptions): string; overload; virtual;
    /// <summary>Composite-aware SELECT ... WHERE over all PK columns, used by
    /// LoadByPKs/GetByPKs. Byte-identical to the single overload for one PK.</summary>
    function CreateSelectByPKSQL(const TableMap: TMVCTableMap): string; overload; virtual;
...
// Per-thread current user for MVCAuditCreatedBy / MVCAuditUpdatedBy. Same
// storage pattern as ActiveRecordConnectionsRegistry.fCurrentConnectionsByThread:
// a TDictionary keyed by TThreadID, guarded by a TMultiReadExclusiveWriteSynchronizer
// so reads are concurrent and only writes take the exclusive lock. Each
// thread only sees its own entry; setting or clearing it in one thread can
// never affect another thread.
var
...
function TMVCActiveRecord.ExecNonQuery(const SQL: string; RefreshAutoGenerated: Boolean;
  RefreshFields: Boolean): int64;
var
  lQry: TFDQuery;
  lPar: TFDParam;
  ...
  lPKAsString: string;
  lOutPar: TFDParam;
  lRowsAffectedSQL: string;
begin
  // Decide whether the statement returns a result set we must consume:
  //   * Insert with autogen-PK without sequence -> RETURNING emits the PK
  //     (and possibly foRefresh fields too).
  //   * The entity's own Insert/Update on a class with foRefresh fields, on
  //     an engine that handles refresh natively -> the statement emits the
  //     foRefresh columns.
  // In either case we Open the query and map the result columns back
  // onto the in-memory instance.
  // The auto-generated PK (at most one, at any position in a composite key)
  // drives the read-back. fAutoGenPKIndex = -1 when none.
  lShouldRefreshPK := RefreshAutoGenerated and
    (fTableMap.fAutoGenPKIndex >= 0) and
    fTableMap.fPrimaryKeys[fTableMap.fAutoGenPKIndex].SequenceName.IsEmpty;
  lShouldRefreshFields := RefreshFields and SQLGenerator.HandlesRefreshNatively and
    (fTableMap.RefreshFields.Count > 0);
  // Engines whose driver cannot be trusted with the row count (SQL Server:
  // a trigger without SET NOCOUNT reports its own) append a query for it.
  lRowsAffectedSQL := '';
  if not (lShouldRefreshPK or lShouldRefreshFields) then
    lRowsAffectedSQL := SQLGenerator.GetRowsAffectedSQL;

  // TFDQuery is used instead of TFDCommand because we may need to Open it
  // as a TDataSet to fetch generated keys, refreshed fields or the row count.
  lQry := CreateQuery(True, True);
  try
    lQry.Connection := GetConnection;
    lSQL := SQL;
    OnBeforeExecuteSQL(lSQL);
    if lRowsAffectedSQL <> '' then
      lSQL := lSQL + ';' + sLineBreak + lRowsAffectedSQL;
    lQry.SQL.Text := lSQL;
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          // lPair.Value is already the TFieldInfo for this field; the old
          // GetInfoByFieldName() was a redundant O(N) scan returning the same
          // object, making the per-save bind loop O(N^2) on the hot path.
          lPar.DataTypeName := lPair.Value.DataTypeName;
          MapTValueToParam(lValue, lPar);
        end
      end;

      // Bind every primary-key column (single-PK: exactly one iteration, identical to before).
      for lPKIdx := 0 to High(fTableMap.fPrimaryKeys) do
      begin
        lPar := lQry.FindParam(SQLGenerator.GetParamNameForSQL(fTableMap.fPrimaryKeys[lPKIdx].FieldName));
        if lPar <> nil then
        begin
          ...
          begin
            ...
            lPar.DataType := fTableMap.fPrimaryKeys[lPKIdx].FieldType;
          end;
          MapTValueToParam(fTableMap.fPrimaryKeys[lPKIdx].RTTIField.GetValue(Self), lPar);
        end;
      end;
    end;
    ...
    begin
      ...
      begin
        // Read the auto-generated PK column back by NAME (robust to column
        // ordering, and correct when the autogen column is not PK[0] in a
        // composite key). Single-PK autogen: same column, same value as before.
        if lShouldRefreshPK then
        begin
          lAutoGenPK := fTableMap.fPrimaryKeys[fTableMap.fAutoGenPKIndex];
          // Type-aware read-back through the shared mapper, like the foRefresh
          // path below and FillPrimaryKey: it tries the Nullable types first
          // and falls through to plain ones - a TGUID is a record too, and
          // calling the Nullable mapper alone silently left it empty.
          // AsInteger would truncate a bigint identity to 32 bits.
          MapDataSetFieldToRTTIField(
            lQry.FieldByName(lAutoGenPK.FieldName),
            lAutoGenPK.RTTIField, Self);
        end;
    ...
    else if lRowsAffectedSQL <> '' then
    begin
      lQry.Open;
      Exit(lQry.Fields[0].AsLargeInt);
    end
    else
    begin
      lQry.ExecSQL(lSQL);
...
var
  ...
  lAutoGenPK: TMVCPKInfo;
begin
  if SequenceName.IsEmpty or (fTableMap.fAutoGenPKIndex < 0) then
    Exit;
  // Sequence pre-fetch targets the (single) auto-generated PK column, wherever
  // it sits in a composite key. Single-PK autogen: identical to before.
  lAutoGenPK := fTableMap.fPrimaryKeys[fTableMap.fAutoGenPKIndex];
  lSQL := SQLGenerator.GetSequenceValueSQL(lAutoGenPK.FieldName, SequenceName);
  ...
  try
    MapDataSetFieldToRTTIField(lDS.Fields[0], lAutoGenPK.RTTIField, Self);
  finally
    lDS.Free;
  end;
end;
...
// Raises an EMVCActiveRecord with a precise message if the field type is not
// compatible with the given audit attribute. Called at InitTableInfo time so
// the problem shows up on the FIRST use of the entity, not at Insert/Update.
procedure CheckAuditDateTimeFieldType(const AField: TRttiField;
  const AClassName, AAttributeName: string);
var
  lTypeInfo: PTypeInfo;
  lNullableType: TNullableType;
begin
  lTypeInfo := AField.FieldType.Handle;
  // Plain TDateTime is fine: same TypeInfo as System.TDateTime.
  if lTypeInfo = TypeInfo(TDateTime) then
    Exit;
  lNullableType := GetNullableType(lTypeInfo);
  if lNullableType in [ntNullableTDateTime, ntNullableTDate] then
    Exit;
  raise EMVCActiveRecord.CreateFmt(
    'Field "%s" in class "%s" is decorated with [%s] but its type "%s" is not supported. ' +
    'Expected: TDateTime, NullableTDateTime or NullableTDate.',
    [AField.Name, AClassName, AAttributeName, AField.FieldType.ToString]);
end;

procedure CheckAuditStringFieldType(const AField: TRttiField;
  const AClassName, AAttributeName: string);
var
  lNullableType: TNullableType;
begin
  if AField.FieldType.TypeKind in [tkUString, tkString, tkLString, tkWString] then
    Exit;
  lNullableType := GetNullableType(AField.FieldType.Handle);
  if lNullableType = ntNullableString then
    Exit;
  raise EMVCActiveRecord.CreateFmt(
    'Field "%s" in class "%s" is decorated with [%s] but its type "%s" is not supported. ' +
    'Expected: string or NullableString.',
    [AField.Name, AClassName, AAttributeName, AField.FieldType.ToString]);
end;

// Scans TMVCTableMap.fProps once to build the validator/audit cache.
// Called from InitTableInfo so the work is done one time per class.
procedure BuildValidatorAndAuditCache(const ATableMap: TMVCTableMap);
var
  lField: TRttiField;
  lAttr: TCustomAttribute;
  lAttrs: TArray<TCustomAttribute>;
  lFieldValidators: TArray<TMVCValidatorBase>;
  lFieldValidatorsCount: Integer;
  lFieldEntry: TMVCFieldValidator;
  lIsPK: Boolean;
  lFieldOptions: TMVCActiveRecordFieldOptions;
  lFieldInfo: TFieldInfo;
  lValidatorsCountTotal: Integer;
begin
  ATableMap.fHasValidators := False;
  ATableMap.fAuditCreatedAtField := nil;
  ATableMap.fAuditUpdatedAtField := nil;
  ATableMap.fAuditCreatedByField := nil;
  ATableMap.fAuditUpdatedByField := nil;
  ATableMap.fSoftDeleteField := nil;
  ATableMap.fSoftDeleteRTTIField := nil;
  ATableMap.fSoftDeleteMode := sdmNone;
  SetLength(ATableMap.fFieldValidators, 0);
  lValidatorsCountTotal := 0;

  for lField in ATableMap.fProps do
  begin
    lAttrs := lField.GetAttributes;
    if Length(lAttrs) = 0 then
      Continue;

    SetLength(lFieldValidators, 0);
    lFieldValidatorsCount := 0;
    lIsPK := ATableMap.TryGetPKOptions(lField, lFieldOptions);
    if not lIsPK then
    begin
      if ATableMap.fMap.TryGetValue(lField, lFieldInfo) then
        lFieldOptions := lFieldInfo.FieldOptions
      else
        lFieldOptions := [];
    end;
    ...
    begin
      ...
      begin
        Inc(lFieldValidatorsCount);
        SetLength(lFieldValidators, lFieldValidatorsCount);
        lFieldValidators[lFieldValidatorsCount - 1] := TMVCValidatorBase(lAttr);
      end
      else if lAttr is MVCAuditCreatedAtAttribute then
      begin
        CheckAuditDateTimeFieldType(lField, ATableMap.fRTTIType.Name, 'MVCAuditCreatedAt');
        ATableMap.fAuditCreatedAtField := lField;
      end
      else if lAttr is MVCAuditUpdatedAtAttribute then
      begin
        CheckAuditDateTimeFieldType(lField, ATableMap.fRTTIType.Name, 'MVCAuditUpdatedAt');
        ATableMap.fAuditUpdatedAtField := lField;
      end
      else if lAttr is MVCAuditCreatedByAttribute then
      begin
        CheckAuditStringFieldType(lField, ATableMap.fRTTIType.Name, 'MVCAuditCreatedBy');
        ATableMap.fAuditCreatedByField := lField;
      end
      else if lAttr is MVCAuditUpdatedByAttribute then
      begin
        CheckAuditStringFieldType(lField, ATableMap.fRTTIType.Name, 'MVCAuditUpdatedBy');
        ATableMap.fAuditUpdatedByField := lField;
      end
      else if lAttr is MVCSoftDeletedAttribute then
      begin
        if ATableMap.fSoftDeleteRTTIField <> nil then
          raise EMVCActiveRecord.CreateFmt(
            'Only one [MVCSoftDeleted] field is allowed per class. ' +
            'Currently field [%s] and field [%s] are both marked.',
            [ATableMap.fSoftDeleteRTTIField.Name, lField.Name]);
        ATableMap.fSoftDeleteRTTIField := lField;
        if ATableMap.fMap.TryGetValue(lField, lFieldInfo) then
          ATableMap.fSoftDeleteField := lFieldInfo;
        if lField.FieldType.Handle = TypeInfo(TDateTime) then
          ATableMap.fSoftDeleteMode := sdmTimestamp
        else if SameText(lField.FieldType.Name, 'NullableTDateTime') then
          ATableMap.fSoftDeleteMode := sdmTimestamp
        else if lField.FieldType.Handle = TypeInfo(Boolean) then
          ATableMap.fSoftDeleteMode := sdmFlag
        else
          raise EMVCActiveRecord.CreateFmt(
            '[MVCSoftDeleted] field [%s] must be TDateTime, NullableTDateTime, or Boolean (got %s)',
            [lField.Name, lField.FieldType.Name]);
      end;
    end;

    if lFieldValidatorsCount > 0 then
    begin
      lFieldEntry.RttiField := lField;
      lFieldEntry.Validators := lFieldValidators;
      lFieldEntry.SkipOnInsert := lIsPK or
        ([foAutoGenerated, foReadOnly, foDoNotInsert] * lFieldOptions <> []);
      Inc(lValidatorsCountTotal);
      SetLength(ATableMap.fFieldValidators, lValidatorsCountTotal);
      ATableMap.fFieldValidators[lValidatorsCountTotal - 1] := lFieldEntry;
    end;
  end;
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            begin
              // Composite PK: each foPrimaryKey field is collected into fPrimaryKeys.
              // The legacy scalar fPrimaryKey* fields mirror the FIRST PK column so every
              // single-PK code path stays byte-identical.
              lPKInfo.RTTIField := lRTTIField;
              lPKInfo.FieldName := MVCTableFieldAttribute(lAttribute).FieldName;
              lPKInfo.Options := MVCTableFieldAttribute(lAttribute).FieldOptions;
              lPKInfo.SequenceName := MVCTableFieldAttribute(lAttribute).SequenceName;
              lPKInfo.InInsert :=
                [foAutoGenerated,foReadOnly,foDoNotInsert] * lPKInfo.Options = [];
              lPrimaryFieldTypeAsStr := lRTTIField.FieldType.ToString.ToLowerInvariant;
              if lPrimaryFieldTypeAsStr.EndsWith('int64') then
              begin
                lPKInfo.FieldType := ftLargeInt;
              end
              ...
              // foRefresh on an auto-generated PK is redundant: the generated value
              // is already read back via RETURNING/OUTPUT (or the sequence fetch).
              // Warn at build time so the misconfiguration is visible at startup
              // (foRefresh is silently ignored for the PK).
              if ([foAutoGenerated, foRefresh] * lPKInfo.Options) = [foAutoGenerated, foRefresh] then
                LogW(Format('ActiveRecord: entity [%s] PK field [%s] declares both foAutoGenerated and foRefresh; ' +
                  'foRefresh is redundant on an auto-generated PK and is ignored.',
                  [ClassName, lPKInfo.FieldName]));

              SetLength(lTableMap.fPrimaryKeys, Length(lTableMap.fPrimaryKeys) + 1);
              lTableMap.fPrimaryKeys[High(lTableMap.fPrimaryKeys)] := lPKInfo;

              if foAutoGenerated in lPKInfo.Options then
              begin
                if lTableMap.fAutoGenPKIndex >= 0 then
                  raise EMVCActiveRecord.Create('A composite primary key may contain at most one auto-generated (foAutoGenerated) field, but entity [' + ClassName + '] declares more than one');
                lTableMap.fAutoGenPKIndex := High(lTableMap.fPrimaryKeys);
              end;

              if lPKCount = 0 then
              begin
                lTableMap.fPrimaryKey := lPKInfo.RTTIField;
                lTableMap.fPrimaryKeyInInsert := lPKInfo.InInsert;
                lTableMap.fPrimaryKeyFieldType := lPKInfo.FieldType;
                lTableMap.fPrimaryKeyFieldName := lPKInfo.FieldName;
                lTableMap.fPrimaryKeyOptions := lPKInfo.Options;
                lTableMap.fPrimaryKeySequenceName := lPKInfo.SequenceName;
              end;
            ...
            if foRefresh in lFieldInfo.FieldOptions then
              lTableMap.fRefreshFields.Add(lFieldInfo);
          end;
        end;
      end;
...
begin
  ...
  Validate(TMVCEntityAction.eaCreate);
  OnValidation(TMVCEntityAction.eaCreate);
  FillAuditFields(TMVCEntityAction.eaCreate);
  OnBeforeInsert;
  OnBeforeInsertOrUpdate;
  if (fTableMap.fMap.WritableFieldsCount = 0) and (not fTableMap.AnyPKInInsert) then
  begin
    raise EMVCActiveRecord.CreateFmt
      ('Cannot insert an entity if no fields are writable. Class [%s] mapped on table [%s]',
      [ClassName, TableName]);
  end;
  // At most one PK column may be auto-generated (enforced at map build). It may
  // sit anywhere in a composite key; its natural siblings are inserted normally.
  if fTableMap.fAutoGenPKIndex >= 0 then
  begin
    lAutoGenPK := fTableMap.fPrimaryKeys[fTableMap.fAutoGenPKIndex];
...
begin
  // Composite-aware overload: for a single-PK table it is byte-identical to the
  // legacy call; for a composite PK it prepends every PK column to the select list.
  Result := SQLGenerator.CreateSelectSQL(fTableMap);
end;
...
class function TMVCActiveRecordHelper.GetByPK<T>(const aValue: int64;
  const RaiseExceptionIfNotFound: Boolean = True): T;
begin
  // ftLargeInt, not ftInteger: on a native-UUID engine (e.g. PostgreSQL) the
  // field-type hint is honored, and binding an Int64 above 32-bit MaxInt as
  // ftInteger overflows the param conversion (silent corruption otherwise).
  Result := GetByPK<T>(aValue.ToString, ftLargeInt, RaiseExceptionIfNotFound);
end;
...
class function TMVCActiveRecordHelper.GetByPKs<T>(const aValues: array of const;
  const RaiseExceptionIfNotFound: Boolean): T;
var
  lFound: Boolean;
begin
  Result := T.Create;
  try
    lFound := Result.LoadByPKs(aValues);
    if not lFound then
    begin
      if RaiseExceptionIfNotFound then
        raise EMVCActiveRecordNotFound.CreateFmt('No data found for key [Entity: %s]', [Result.ClassName])
      else
        FreeAndNil(Result);
    end;
  except
...
begin
  ...
  begin
    // Reserve the leading slots for ALL primary-key columns (they are not part
    // of fMap). Single-PK: exactly one leading slot, identical to before.
    SetLength(fTableMap.fMapping, fTableMap.fMap.Count + Length(fTableMap.fPrimaryKeys));
    for I := 0 to High(fTableMap.fPrimaryKeys) do
    begin
      lParentType := fTableMap.fPrimaryKeys[I].RTTIField.Parent;
      fTableMap.fMapping[I].InstanceFieldName := fTableMap.fPrimaryKeys[I].RTTIField.Name.Substring(1).ToLower;
      fTableMap.fMapping[I].DatabaseFieldName := fTableMap.fPrimaryKeys[I].FieldName;
      lPropFromField := lParentType.GetProperty(fTableMap.fPrimaryKeys[I].RTTIField.Name.Substring(1));
    ...
    I := Length(fTableMap.fPrimaryKeys);
...
begin
  if fTableMap.IsCompositePK then
    raise EMVCActiveRecord.Create(
      'Entity has a composite primary key; use GetPKs instead of GetPK');
...
begin
  { Reading fPrimaryKeyOptions, the mirror of column [0], answered False for a
    composite key whose generated column sits anywhere else. fAutoGenPKIndex
    knows where it is, and is honest for every shape: no key at all, one key
    column, or several. This getter must not raise instead - serialization
    reads it on entities of any shape, including ones with no primary key. }
  if fTableMap = nil then
    raise EMVCActiveRecord.Create('Entity has no table mapping');
  Result := fTableMap.fAutoGenPKIndex >= 0;
end;
...
class procedure TMVCActiveRecord.SetCurrentUser(const AUserName: string);
var
  lTid: TThreadID;
begin
  lTid := TThread.CurrentThread.ThreadID;
  gCurrentUsersLock.BeginWrite;
  try
    if AUserName = '' then
      gCurrentUsersByThread.Remove(lTid)
    else
      gCurrentUsersByThread.AddOrSetValue(lTid, AUserName);
  finally
    gCurrentUsersLock.EndWrite;
  end;
end;

class function TMVCActiveRecord.GetCurrentUser: string;
begin
  gCurrentUsersLock.BeginRead;
  try
    if not gCurrentUsersByThread.TryGetValue(TThread.CurrentThread.ThreadID, Result) then
      Result := '';
  finally
    gCurrentUsersLock.EndRead;
  end;
end;
...
begin
  lAR := aClass.Create;
  try
    lSQL := lAR.SQLGenerator.CreateDeleteAllSQL(lAR.fTableMap.fTableName) + lAR.SQLGenerator.GetDefaultSQLFilter(True);
...
begin
  ...
  begin
    ...
    // Map every PK column back (they are not part of fMap). Single-PK: one iteration.
    for lPKIdx := 0 to High(fTableMap.fPrimaryKeys) do
    begin
      MapDataSetFieldToRTTIField(
        aDataSet.FieldByName(fTableMap.fPrimaryKeys[lPKIdx].FieldName),
        fTableMap.fPrimaryKeys[lPKIdx].RTTIField, Self);
    end;
  end;
  OnAfterLoad;
  RebuildHashSnapshot;
...
begin
  if fTableMap.IsCompositePK then
    raise EMVCActiveRecord.Create(
      'Entity has a composite primary key; use LoadByPKs (one value per PK column) instead of LoadByPK');
  CheckAction(TMVCEntityAction.eaRetrieve);
...
function TMVCActiveRecord.DoLoadByPKSelect(const aParams: array of Variant;
  const aParamTypes: array of TFieldType): Boolean;
var
  lSQL: string;
  lDataSet: TDataSet;
begin
  CheckAction(TMVCEntityAction.eaRetrieve);
  lSQL := SQLGenerator.CreateSelectByPKSQL(fTableMap) +
    SQLGenerator.BuildSoftDeleteWhereSuffix(fTableMap);
  OnBeforeExecuteSQL(lSQL);
  lDataSet := ExecQuery(lSQL, aParams, aParamTypes, GetConnection, True, False);
  try
    Result := not lDataSet.Eof;
    if Result then
    begin
...
function TMVCActiveRecord.LoadByPKs(const aValues: array of const): Boolean;
var
  lParams: array of Variant;
  lParamTypes: array of TFieldType;
  I: Integer;
begin
  if not fTableMap.HasPK then
    raise EMVCActiveRecord.Create('No primary key defined');
  if Length(aValues) <> Length(fTableMap.fPrimaryKeys) then
    raise EMVCActiveRecord.CreateFmt(
      'LoadByPKs expects %d value(s) for the primary key of [%s], got %d',
      [Length(fTableMap.fPrimaryKeys), ClassName, Length(aValues)]);
  SetLength(lParams, Length(aValues));
  SetLength(lParamTypes, Length(aValues));
  for I := 0 to High(aValues) do
  begin
    lParams[I] := VarRecToVariantForPK(aValues[I], fTableMap.fPrimaryKeys[I].FieldType);
    lParamTypes[I] := fTableMap.fPrimaryKeys[I].FieldType;
  end;
  Result := DoLoadByPKSelect(lParams, lParamTypes);
end;

function TMVCActiveRecord.LoadByPKs(const aValues: TArray<string>): Boolean;
var
  lParams: array of Variant;
  lParamTypes: array of TFieldType;
  I: Integer;
...
function TMVCActiveRecord.PKColumnValueAsString(const AIndex: Integer): string;
var
  lValue: TValue;
begin
  lValue := fTableMap.fPrimaryKeys[AIndex].RTTIField.GetValue(Self);
  if lValue.Kind = tkRecord then
  begin
    // Nullable PK column: unwrap the common composite-PK nullable types.
    if lValue.IsType<NullableInt64> then
      Result := lValue.AsType<NullableInt64>().Value.ToString
    else if lValue.IsType<NullableInt32> then
      Result := lValue.AsType<NullableInt32>().Value.ToString
    else if lValue.IsType<NullableString> then
      Result := lValue.AsType<NullableString>().Value
    else if lValue.IsType<NullableTGUID> then
      Result := lValue.AsType<NullableTGUID>().Value.ToString
    else
...
function TMVCActiveRecord.PKAsJSONArray: string;
...
begin
  ...
  begin
    if I > 0 then
      Result := Result + ',';
    lPart := PKColumnValueAsString(I);
    // Integer columns emit a bare JSON number; string/GUID columns emit a quoted,
    // escaped JSON string, so any character (including ; , ] ") is unambiguous.
    if fTableMap.fPrimaryKeys[I].FieldType in [ftInteger, ftLargeInt] then
      Result := Result + lPart
    else
      Result := Result + '"' + JsonEscape(lPart) + '"';
  end;
  Result := Result + ']';
end;

function TMVCActiveRecord.PKAsURLSegment: string;
begin
  if fTableMap.IsCompositePK then
    Result := PKAsJSONArray
  else if fTableMap.HasPK then
    Result := PKColumnValueAsString(0)
  else
    Result := '';
end;
...
procedure TMVCActiveRecord.SetAuditDateTimeField(const AField: TRttiField; const AValue: TDateTime);
var
  lNullableType: TNullableType;
  lNullableDT: NullableTDateTime;
  lNullableD: NullableTDate;
begin
  lNullableType := GetNullableType(AField.FieldType.Handle);
  case lNullableType of
    ntNullableTDateTime:
      begin
        lNullableDT := AValue;
        AField.SetValue(Self, TValue.From<NullableTDateTime>(lNullableDT));
      end;
    ntNullableTDate:
      begin
        lNullableD := AValue;
        AField.SetValue(Self, TValue.From<NullableTDate>(lNullableD));
      end;
  else
    AField.SetValue(Self, TValue.From<TDateTime>(AValue));
  end;
end;

procedure TMVCActiveRecord.SetAuditStringField(const AField: TRttiField; const AValue: string);
var
  lNullableType: TNullableType;
  lNullableStr: NullableString;
begin
  lNullableType := GetNullableType(AField.FieldType.Handle);
  case lNullableType of
    ntNullableString:
      begin
        lNullableStr := AValue;
        AField.SetValue(Self, TValue.From<NullableString>(lNullableStr));
      end;
  else
    AField.SetValue(Self, TValue.From<string>(AValue));
  end;
end;

procedure TMVCActiveRecord.FillAuditFields(const EntityAction: TMVCEntityAction);
var
  lNow: TDateTime;
  lUser: string;
begin
  if (fTableMap.fAuditCreatedAtField = nil) and (fTableMap.fAuditUpdatedAtField = nil)
    and (fTableMap.fAuditCreatedByField = nil) and (fTableMap.fAuditUpdatedByField = nil) then
    Exit;
  ...
  case EntityAction of
    eaCreate:
      begin
        if fTableMap.fAuditCreatedAtField <> nil then
          SetAuditDateTimeField(fTableMap.fAuditCreatedAtField, lNow);
        if fTableMap.fAuditUpdatedAtField <> nil then
          SetAuditDateTimeField(fTableMap.fAuditUpdatedAtField, lNow);
        lUser := TMVCActiveRecord.GetCurrentUser;
        if lUser <> '' then
        begin
          if fTableMap.fAuditCreatedByField <> nil then
            SetAuditStringField(fTableMap.fAuditCreatedByField, lUser);
          if fTableMap.fAuditUpdatedByField <> nil then
            SetAuditStringField(fTableMap.fAuditUpdatedByField, lUser);
        end;
      end;
    eaUpdate:
      begin
        if fTableMap.fAuditUpdatedAtField <> nil then
          SetAuditDateTimeField(fTableMap.fAuditUpdatedAtField, lNow);
        lUser := TMVCActiveRecord.GetCurrentUser;
        if (lUser <> '') and (fTableMap.fAuditUpdatedByField <> nil) then
          SetAuditStringField(fTableMap.fAuditUpdatedByField, lUser);
      end;
  end;
end;
...
procedure TMVCActiveRecord.OnStorageValidate(const AErrors: PMVCValidationErrors;
  const EntityAction: TMVCEntityAction);
begin
  // no-op: override to collect business/cross-field errors on save.
end;

// Unwraps a Nullable<T> TValue into its underlying primitive TValue so that
// validators that don't understand tkRecord (MVCEmail, MVCMaxLength, MVCRange,
// ...) can inspect the real value. Null (HasValue=false) returns an empty
// TValue so validators that treat IsEmpty as "skip" still behave correctly.
// The NullableXxx records expose properties but not via RTTI, so we read the
// backing fValue/fHasValue fields directly (fHasValue = '_' when set).
function UnwrapNullableValue(const AValue: TValue): TValue;
var
  lCtx: TRttiContext;
  lType: TRttiType;
...
begin
  Result := AValue;
  if (AValue.TypeInfo = nil) or (AValue.Kind <> tkRecord) then
    Exit;
  lTypeName := string(AValue.TypeInfo.Name);
  if not lTypeName.StartsWith('Nullable', True) then
    Exit;
  ...
  try
    ...
    if lType = nil then
      Exit;
    lHasValueField := lType.GetField('fHasValue');
    lValueField := lType.GetField('fValue');
    if (lHasValueField = nil) or (lValueField = nil) then
      Exit;
    if lHasValueField.GetValue(AValue.GetReferenceToRawData).AsString = '_' then
      Result := lValueField.GetValue(AValue.GetReferenceToRawData)
    else
      Result := TValue.Empty;
  finally
    lCtx.Free;
...
procedure TMVCActiveRecord.CollectFieldValidatorErrors(
  const AErrors: PMVCValidationErrors; const EntityAction: TMVCEntityAction);
var
  lFieldValidator: TMVCFieldValidator;
  lValidator: TMVCValidatorBase;
  lValue, lUnwrappedValue: TValue;
  lFieldName: string;
begin
  if not fTableMap.fHasValidators then
    Exit;
  for lFieldValidator in fTableMap.fFieldValidators do
  begin
    if (EntityAction = eaCreate) and lFieldValidator.SkipOnInsert then
      Continue;
    lValue := lFieldValidator.RttiField.GetValue(Self);
    lUnwrappedValue := UnwrapNullableValue(lValue);
    lFieldName := lFieldValidator.RttiField.Name;
    if lFieldName.StartsWith('f') or lFieldName.StartsWith('F') then
      lFieldName := lFieldName.Substring(1);
    for lValidator in lFieldValidator.Validators do
    begin
      if not lValidator.Validate(lUnwrappedValue, Self) then
        AErrors.Add(lFieldName, lValidator.GetErrorMessage);
    end;
  end;
end;

procedure TMVCActiveRecord.Validate(const EntityAction: TMVCEntityAction);
var
  lErrors: TMVCValidationErrors;
begin
  FillChar(lErrors, SizeOf(lErrors), 0);
  try
    CollectFieldValidatorErrors(@lErrors, EntityAction);
    OnStorageValidate(@lErrors, EntityAction);
    // The exception constructor COPIES the dictionary, so we hand over the
    // reference and let the outer finally Clear the original.
    if lErrors.HasErrors then
      raise EMVCStorageValidationException.Create(lErrors.Errors);
  finally
    lErrors.Clear;
  end;
end;

function TMVCActiveRecord.ReloadByCurrentPK: Boolean;
var
  lParams: array of Variant;
  lParamTypes: array of TFieldType;
  I: Integer;
begin
  ...
  if fTableMap.IsCompositePK then
  begin
    // Reload by the CURRENT value of every PK column. The caller sets the key
    // through the entity's typed properties (by name), so this path never
    // depends on the field declaration order. Reuses the shared select+bind.
    SetLength(lParams, Length(fTableMap.fPrimaryKeys));
    SetLength(lParamTypes, Length(fTableMap.fPrimaryKeys));
    for I := 0 to High(fTableMap.fPrimaryKeys) do
    begin
  ...
  begin
    case GetPrimaryKeyFieldType of
      ftLargeInt:
        Result := LoadByPK(GetPK.AsInt64);
      ftInteger:
        Result := LoadByPK(GetPK.AsInteger);
      ftString:
        Result := LoadByPK(GetPK.AsString);
      ftGuid:
        Result := LoadByPK(GetPK.AsType<TGUID>);
      else
...
function TMVCActiveRecord.Refresh(const RaiseExceptionIfNotFound: Boolean): Boolean;
begin
  // Re-read the row by the current PK, discarding any unsaved local edits.
  // Same operation as Load: raises EMVCActiveRecordNotFound when the row is
  // gone (silently keeping the stale in-memory copy was a bug). Pass
  // RaiseExceptionIfNotFound = False when a missing row is an expected outcome.
  Result := Load(RaiseExceptionIfNotFound);
end;

function TMVCActiveRecord.Load(const RaiseExceptionIfNotFound: Boolean): Boolean;
begin
  // Fail-loud reload by the key currently set on the fields. Order-independent
  // (the key is assigned by property name). A missing row raises by default so
  // it can never masquerade as stale-but-present data.
  Result := ReloadByCurrentPK;
  if (not Result) and RaiseExceptionIfNotFound then
    raise EMVCActiveRecordNotFound.CreateFmt('No data found for key [Entity: %s][PK: %s]',
      [ClassName, PKAsURLSegment]);
end;
...
class function TMVCActiveRecordHelper.SelectUnidirectionalDataSet(const SQL: string;
  const Params: array of Variant; const DirectExecute: Boolean): TDataSet;
begin
  Result := SelectDataSet(SQL, Params, {Unidirectional} True, DirectExecute);
end;
...
class function TMVCActiveRecordHelper.SelectUnidirectionalDataSet(const SQL: string;
  ...
  const DirectExecute: Boolean): TDataSet;
begin
  Result := SelectDataSet(SQL, Params, ParamTypes, {Unidirectional} True, DirectExecute);
end;
...
function TMVCActiveRecord.VarRecToVariantForPK(const AItem: TVarRec;
  const AFieldType: TFieldType): Variant;
begin
  // Mirror the single-value LoadByPK pipeline: pass the value as a string and let
  // the declared FieldType drive FireDAC's coercion. Keeps behaviour consistent
  // with the proven Int64/string/GUID single-PK path.
  // System.vt* qualified: MVCFramework.RQL.Parser also declares a vtInteger/vtString
  // (TRQLValueType) that would otherwise shadow the TVarRec tag constants.
  case AItem.VType of
    System.vtInteger: Result := IntToStr(AItem.VInteger);
    System.vtInt64: Result := IntToStr(AItem.VInt64^);
    System.vtUnicodeString: Result := string(AItem.VUnicodeString);
    System.vtWideString: Result := WideString(AItem.VWideString);
{$IFNDEF NEXTGEN}
    System.vtAnsiString: Result := string(AnsiString(AItem.VAnsiString));
    System.vtString: Result := string(AItem.VString^);
{$ENDIF}
    System.vtChar: Result := string(AItem.VChar);
    System.vtWideChar: Result := AItem.VWideChar;
  else
    raise EMVCActiveRecord.Create('Unsupported value type for primary key column');
  end;
end;

function TMVCActiveRecord.VarRecToTValueForPK(const APKField: TRTTIField;
  const AItem: TVarRec; const AFieldType: TFieldType): TValue;
...
var
  ...
  lI64: Int64;
begin
  lIsNullable := APKField.GetValue(Self).Kind = tkRecord;
  case AFieldType of
    ftInteger, ftLargeInt:
      begin
        case AItem.VType of
          System.vtInteger: lI64 := AItem.VInteger;
          System.vtInt64: lI64 := AItem.VInt64^;
        else
          lI64 := StrToInt64(AsStr);
        end;
        if AFieldType = ftLargeInt then
          Result := TValue.From<Int64>(lI64)
        else
          Result := TValue.From<Integer>(Integer(lI64));
      end;
    ftString:
      begin
        if lIsNullable then
          Result := TValue.From<NullableString>(AsStr)
        else
          Result := TValue.From<string>(AsStr);
      end;
    ftGuid:
      begin
        Result := TValue.From<TGUID>(StringToGUID(AsStr));
      end;
  else
    raise EMVCActiveRecord.Create('Unsupported primary key field type');
  end;
end;

procedure TMVCActiveRecord.SetPKField(const APKField: TRTTIField; const aValue: TValue);
var
  lPKValue: TValue;
begin
  if APKField.GetValue(Self).Kind = tkRecord then
  begin
    lPKValue := APKField.GetValue(Self);
    ...
    APKField.SetValue(Self, lPKValue);
  end
  else
  begin
    APKField.SetValue(Self, aValue)
  end;
end;
...
begin
  ...
  if fTableMap.IsCompositePK then
    raise EMVCActiveRecord.Create(
      'Entity has a composite primary key; use SetPKs instead of SetPK');
  SetPKField(fTableMap.fPrimaryKey, aValue);
end;

procedure TMVCActiveRecord.SetPKs(const aValues: array of const);
var
  I: Integer;
  lVal: TValue;
begin
  ...
  if Length(aValues) <> Length(fTableMap.fPrimaryKeys) then
    raise EMVCActiveRecord.CreateFmt(
      'SetPKs expects %d value(s) for the composite primary key of [%s], got %d',
      [Length(fTableMap.fPrimaryKeys), ClassName, Length(aValues)]);
  for I := 0 to High(fTableMap.fPrimaryKeys) do
  begin
    lVal := VarRecToTValueForPK(fTableMap.fPrimaryKeys[I].RTTIField, aValues[I],
      fTableMap.fPrimaryKeys[I].FieldType);
    SetPKField(fTableMap.fPrimaryKeys[I].RTTIField, lVal);
  end;
end;

procedure TMVCActiveRecord.SetPKs(const aValues: TArray<string>);
var
  I: Integer;
  lVal: TValue;
  lVarRec: TVarRec;
begin
  ...
  for I := 0 to High(fTableMap.fPrimaryKeys) do
  begin
    // Reuse the typed converter via a string TVarRec so nullable/plain PK
    // columns are handled uniformly.
    lVarRec.VType := System.vtUnicodeString;
    lVarRec.VUnicodeString := Pointer(aValues[I]);
    lVal := VarRecToTValueForPK(fTableMap.fPrimaryKeys[I].RTTIField, lVarRec,
      fTableMap.fPrimaryKeys[I].FieldType);
    SetPKField(fTableMap.fPrimaryKeys[I].RTTIField, lVal);
  end;
end;

function TMVCActiveRecord.GetPKs: TArray<TValue>;
var
  I: Integer;
begin
  SetLength(Result, Length(fTableMap.fPrimaryKeys));
  for I := 0 to High(fTableMap.fPrimaryKeys) do
    Result[I] := fTableMap.fPrimaryKeys[I].RTTIField.GetValue(Self);
end;

function TMVCActiveRecord.HasCompositePK: Boolean;
begin
  Result := fTableMap.IsCompositePK;
end;

procedure TMVCActiveRecord.CheckSinglePKForAutogenFlag;
begin
  { Writing the flag is the direction that must fail loudly: there has to be
    exactly one key column for "the primary key is auto-generated" to name
    something. With no key there is nothing to mark, and with a composite key
    the setter cannot know which of the columns you meant - guessing column [0]
    would leave the caller believing it had switched a behaviour it had not. }
  if fTableMap = nil then
    raise EMVCActiveRecord.Create('Entity has no table mapping');
  if not fTableMap.HasPK then
    raise EMVCActiveRecord.Create(
      'Entity has no primary key; there is no column to mark as auto-generated');
  if fTableMap.IsCompositePK then
    raise EMVCActiveRecord.Create(
      'Entity has a composite primary key; PrimaryKeyIsAutogenerated cannot tell which column you mean, ' +
      'declare foAutoGenerated on the specific column instead');
end;

procedure TMVCActiveRecord.SetPrimaryKeyIsAutogenerated(const Value: Boolean);
begin
  CheckSinglePKForAutogenFlag;
  { fPrimaryKeyOptions is only a mirror of fPrimaryKeys[0]. Writing the mirror
    alone - which is all this setter used to do - left fAutoGenPKIndex and
    fPrimaryKeys[0] untouched, and those are what Insert actually reads: the
    caller was told it had switched a behaviour that never switched. All four
    now move together.
    Note this edits the table map, which is built once per class and shared by
    every instance of it. }
  if Value then
  begin
    Include(fTableMap.fPrimaryKeyOptions, foAutoGenerated);
    Include(fTableMap.fPrimaryKeys[0].Options, foAutoGenerated);
    fTableMap.fAutoGenPKIndex := 0;
  end
  else
  begin
    Exclude(fTableMap.fPrimaryKeyOptions, foAutoGenerated);
    Exclude(fTableMap.fPrimaryKeys[0].Options, foAutoGenerated);
    fTableMap.fAutoGenPKIndex := -1;
  end;
  fTableMap.fPrimaryKeys[0].InInsert :=
    [foAutoGenerated, foReadOnly, foDoNotInsert] * fTableMap.fPrimaryKeys[0].Options = [];
end;
...
begin
  ...
  lAffectedRows := ExecNonQuery(SQL, False, True);
  if (lAffectedRows = 0) and RaiseExceptionIfNotFound then
  begin
    if fTableMap.fIsVersioned then
    begin
...
begin
  ...
  begin
    if NewList[0].HasCompositePK then
      raise EMVCActiveRecord.Create(
        'Merge is not supported for entities with a composite primary key');
...
begin
  Result := CreateDeleteAllSQL(TableMap.fTableName) + ' WHERE ' +
    BuildPKWhereConjunction(TableMap, '=:');
...
function TMVCSQLGenerator.CreateSelectByPKSQL(const TableMap: TMVCTableMap): string;
begin
  if not TableMap.HasPK then
  begin
    raise EMVCActiveRecord.Create
      ('No primary key provided. [HINT] Define a primary key field adding foPrimaryKey in field options.');
  end;
  Result := CreateSelectSQL(TableMap) + ' WHERE ' + BuildPKWhereConjunction(TableMap, '= :') +
...
function TMVCSQLGenerator.CreateSelectSQL(const TableMap: TMVCTableMap): string;
begin
  Result := 'SELECT ' + TableFieldsDelimited(TableMap.fMap, TableMap, ',') + ' FROM ' +
    GetTableNameForSQL(TableMap.fTableName);
end;
...
//  end;
  ...
  if TableMap.HasPK then
  begin
    Result := Result + ' where ' + BuildPKWhereConjunction(TableMap, '= :');
    if TableMap.fIsVersioned then
    begin
...
function TMVCSQLGenerator.BuildReturningColumnList(const TableMap: TMVCTableMap): string;
var
  lFieldInfo: TFieldInfo;
  lFirst: Boolean;
begin
  Result := '';
  lFirst := True;
  // Emit the auto-generated PK column (wherever it sits in a composite key) first,
  // so ExecNonQuery can read it back. Single-PK autogen: identical to before.
  if TableMap.fAutoGenPKIndex >= 0 then
  begin
    Result := AutoGenPKFieldName(TableMap);
    lFirst := False;
  end;
  for lFieldInfo in TableMap.RefreshFields do
  begin
    if lFirst then
      Result := GetFieldNameForSQL(lFieldInfo.FieldName)
    else
      Result := Result + ', ' + GetFieldNameForSQL(lFieldInfo.FieldName);
    lFirst := False;
  end;
end;
...
function TMVCSQLGenerator.GetRowsAffectedSQL: string;
begin
  Result := '';
end;
...
function TMVCSQLGenerator.TableFieldsDelimited(const Map: TFieldsMap; const TableMap: TMVCTableMap;
  const Delimiter: string): string;
var
  lPair: TPair<TRTTIField, TFieldInfo>;
  lPKList: string;
...
begin
  ...
  Result := Copy(Result, 1, Length(Result) - Length(Delimiter));
  // Prepend ALL primary-key columns (they are not part of fMap) in declaration order.
  lPKList := '';
  for I := 0 to High(TableMap.fPrimaryKeys) do
  begin
    if I > 0 then
      lPKList := lPKList + ', ';
    lPKList := lPKList + GetFieldNameForSQL(TableMap.fPrimaryKeys[I].FieldName);
  end;
  if not lPKList.IsEmpty then
  begin
    if not Result.IsEmpty then
      Result := lPKList + ', ' + Result
    else
      Result := lPKList;
  end;
end;

function TMVCSQLGenerator.BuildPKWhereConjunction(const TableMap: TMVCTableMap;
  const AEqualsSep: string): string;
var
  I: Integer;
  lName: string;
begin
  ...
  for I := 0 to High(TableMap.fPrimaryKeys) do
  begin
    lName := TableMap.fPrimaryKeys[I].FieldName;
    if I > 0 then
      Result := Result + ' and ';
    Result := Result + GetFieldNameForSQL(lName) + AEqualsSep + GetParamNameForSQL(lName);
  end;
end;

function TMVCSQLGenerator.PKInsertColumns(const TableMap: TMVCTableMap): string;
var
  I: Integer;
begin
  Result := '';
  for I := 0 to High(TableMap.fPrimaryKeys) do
    if TableMap.fPrimaryKeys[I].InInsert then
      Result := Result + GetFieldNameForSQL(TableMap.fPrimaryKeys[I].FieldName) + ',';
end;

function TMVCSQLGenerator.PKInsertParams(const TableMap: TMVCTableMap): string;
var
  I: Integer;
begin
  Result := '';
  for I := 0 to High(TableMap.fPrimaryKeys) do
    if TableMap.fPrimaryKeys[I].InInsert then
      Result := Result + ':' + GetParamNameForSQL(TableMap.fPrimaryKeys[I].FieldName) + ',';
end;

function TMVCSQLGenerator.AutoGenPKFieldName(const TableMap: TMVCTableMap): string;
begin
  if TableMap.fAutoGenPKIndex >= 0 then
    Result := GetFieldNameForSQL(TableMap.fPrimaryKeys[TableMap.fAutoGenPKIndex].FieldName)
  else
    Result := '';
end;
...
begin
  ...
  // Composite PKs are not merged (Merge guards them) so no single-value dispatch.
  if Value.HasCompositePK then
    lFound := False
  else
...
begin
  ...
  fSoftDeleteRTTIField := nil;
  fSoftDeleteMode := sdmNone;
  fAutoGenPKIndex := -1;
end;

function TMVCTableMap.IsCompositePK: Boolean;
begin
  Result := Length(fPrimaryKeys) > 1;
end;

function TMVCTableMap.HasPK: Boolean;
begin
  Result := Length(fPrimaryKeys) > 0;
end;

function TMVCTableMap.AnyPKInInsert: Boolean;
var
  I: Integer;
begin
  for I := 0 to High(fPrimaryKeys) do
    if fPrimaryKeys[I].InInsert then
      Exit(True);
  Result := False;
end;

function TMVCTableMap.TryGetPKOptions(const AField: TRTTIField;
  out AOptions: TMVCActiveRecordFieldOptions): Boolean;
var
  I: Integer;
begin
  for I := 0 to High(fPrimaryKeys) do
    if fPrimaryKeys[I].RTTIField = AField then
    begin
      AOptions := fPrimaryKeys[I].Options;
      Exit(True);
    end;
  Result := False;
end;
...
function TMVCTableMap.IsSoftDeleteEnabled: Boolean;
begin
  Result := fSoftDeleteMode <> sdmNone;
end;
...
begin
  ...
  // Unidirectional queries are meant for forward-only streaming: pair
  // them with fmOnDemand so rows are fetched in rowset-sized windows
  // as the consumer calls Next, instead of being prefetched in bulk
  // at Open (which would materialise the whole result in the driver
  // buffer even though the caller can't scroll back).
...
gCurrentUsersByThread := TDictionary<TThreadID, string>.Create;
gCurrentUsersLock := TMultiReadExclusiveWriteSynchronizer.Create;
...
gCtx.Free;
gCurrentUsersLock.Free;
gCurrentUsersByThread.Free;
gTableMapLock.Free;
