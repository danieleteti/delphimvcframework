...
type
  ...
  TMVCActiveRecordFieldOption = (foAutoGenerated, foTransient);
  ...
  public
    ...
    constructor Create(const aFieldName: string;
      const aFieldOptions: TMVCActiveRecordFieldOptions); overload;
  ...
  private
    ...
    fRQL2SQL: TRQL2SQL;
    procedure MapColumnToTValue(const aFieldName: string; const aField: TField;
  ...
  protected
    ...
    function ExecNonQuery(const SQL: string;
      RefreshAutoGenerated: Boolean = false): int64; overload;
    // load events
    /// <summary>
  ...
  public
    ...
    class function GetByPK<T: TMVCActiveRecord, constructor>
      (const aValue: int64): T; overload;
    class function GetByPK(const aClass: TMVCActiveRecordClass;
      const aValue: int64): TMVCActiveRecord; overload;
    class function GetScalar(const SQL: string;
    ...
    class function Select<T: TMVCActiveRecord, constructor>(const SQL: string;
      ...
      overload;
    class function Select(const aClass: TMVCActiveRecordClass;
    ...
    class function SelectRQL(const aClass: TMVCActiveRecordClass;
      ...
      : TMVCActiveRecordList; overload;
    class function DeleteRQL(const aClass: TMVCActiveRecordClass;
    ...
    function SelectRQL(const RQL: string; const MaxRecordCount: Integer)
      : TMVCActiveRecordList; overload;
    class function Where<T: TMVCActiveRecord, constructor>(const SQLWhere
      : string; const Params: array of Variant): TObjectList<T>; overload;
    ...
    class function GetFirstByWhere<T: TMVCActiveRecord, constructor>
      (const SQLWhere: string; const Params: array of Variant;
      const RaiseExceptionIfNotFound: Boolean = True): T;
    class function Where(const aClass: TMVCActiveRecordClass;
      const SQLWhere: string; const Params: array of Variant)
      : TMVCActiveRecordList; overload;
    class function Where(const aClass: TMVCActiveRecordClass;
      const SQLWhere: string; const Params: array of Variant;
      const Connection: TFDConnection): TMVCActiveRecordList; overload;
    class function All<T: TMVCActiveRecord, constructor>
      : TObjectList<T>; overload;
    class function All(const aClass: TMVCActiveRecordClass)
      : TObjectList<TMVCActiveRecord>; overload;
    class function DeleteAll(const aClass: TMVCActiveRecordClass)
      : int64; overload;
    ...
    class function SelectDataSet(const SQL: string;
  ...
  IMVCEntitiesRegistry = interface
    ...
    procedure AddEntity(const aURLSegment: string;
  ...
  IMVCActiveRecordConnections = interface
    ...
    procedure AddConnection(const aName: string;
      const aConnection: TFDConnection; const Owns: Boolean = false);
  ...
  TMVCConnectionsRepository = class(TInterfacedObject,
  ...
  public
    ...
    destructor Destroy; override;
    procedure AddConnection(const aName: string;
      const aConnection: TFDConnection; const aOwns: Boolean = false);
    procedure RemoveConnection(const aName: string);
    procedure SetCurrent(const aName: string);
    function GetCurrent: TFDConnection;
  ...
  protected
    ...
    function GetMapping: TMVCFieldsMapping;
    function TableFieldsDelimited(const Map: TDictionary<TRttiField, string>;
      const PKFieldName: string; const Delimiter: string): string;
  public
    constructor Create(Mapping: TMVCFieldsMapping); virtual;
    destructor Destroy; override;
    function CreateSQLWhereByRQL(const RQL: string;
      const Mapping: TMVCFieldsMapping;
      const UseArtificialLimit: Boolean = True): string; virtual; abstract;
    function CreateSelectSQL(const TableName: string;
    ...
    function CreateSelectByPKSQL(const TableName: string;
      const Map: TDictionary<TRttiField, string>; const PKFieldName: string;
      const PKOptions: TMVCActiveRecordFieldOptions;
      const PrimaryKeyValue: int64): string; virtual; abstract;
    function CreateInsertSQL(const TableName: string;
      const Map: TDictionary<TRttiField, string>; const PKFieldName: string;
      const PKOptions: TMVCActiveRecordFieldOptions): string; virtual; abstract;
    function CreateUpdateSQL(const TableName: string;
      const Map: TDictionary<TRttiField, string>; const PKFieldName: string;
      const PKOptions: TMVCActiveRecordFieldOptions): string; virtual; abstract;
    function CreateDeleteSQL(const TableName: string;
      const Map: TDictionary<TRttiField, string>; const PKFieldName: string;
      const PKOptions: TMVCActiveRecordFieldOptions;
      const PrimaryKeyValue: int64): string; virtual; abstract;
    function CreateDeleteAllSQL(const TableName: string): string;
      virtual; abstract;
    function CreateSelectCount(const TableName: String): String;
      virtual; abstract;
  end;
...
begin
  ...
  try
    ...
    if (lName = 'default') and
      (not fCurrentConnectionsByThread.ContainsKey
      (TThread.CurrentThread.ThreadID)) then
    begin
      fCurrentConnectionsByThread.AddOrSetValue
        (TThread.CurrentThread.ThreadID, lName);
    end;
  finally
    fMREW.EndWrite;
  end;
...
function TMVCConnectionsRepository.GetByName(const aName: string)
  : TFDConnection;
var
  lKeyName: string;
  lConnHolder: TConnHolder;
...
begin
  fMREW.BeginRead;
  try
    if fCurrentConnectionsByThread.TryGetValue(TThread.Current.ThreadID, lName)
    then
    begin
...
begin
  ...
  try
    ...
    if RefreshAutoGenerated and (TMVCActiveRecordFieldOption.foAutoGenerated
      in fPrimaryKeyOptions) then
    begin
      lQry.Open;
      fPrimaryKey.SetValue(Self, lQry.FieldByName(fPrimaryKeyFieldName)
        .AsInteger);
      OnAfterLoad;
    end
    else
...
class function TMVCActiveRecord.ExecQuery(const SQL: string;
  const Values: array of Variant): TDataSet;
begin
...
begin
  fEntityAllowedActions := [TMVCEntityAction.eaCreate,
    TMVCEntityAction.eaRetrieve, TMVCEntityAction.eaUpdate,
  ...
  begin
    ...
    begin
      fEntityAllowedActions := MVCEntityActionsAttribute(lAttribute)
  ...
  begin
    ...
    begin
      ...
      begin
        fMap.Add(lRTTIField, { fTableName + '.' + } MVCTableFieldAttribute
          (lAttribute).FieldName);
        if not(foTransient in MVCTableFieldAttribute(lAttribute).FieldOptions)
        then
          fMapNonTransientFields.Add(lRTTIField,
            MVCTableFieldAttribute(lAttribute).FieldName);
      end
...
begin
  ...
  begin
    raise EMVCActiveRecord.CreateFmt
      ('Cannot insert an entity if all fields are transient. Class [%s] mapped on table [%s]',
      [ClassName, fTableName]);
  end;
  SQL := SQLGenerator.CreateInsertSQL(fTableName, fMapNonTransientFields,
    fPrimaryKeyFieldName, fPrimaryKeyOptions);
...
class function TMVCActiveRecord.GetByPK(const aClass: TMVCActiveRecordClass;
  const aValue: int64): TMVCActiveRecord;
begin
...
class function TMVCActiveRecord.GetOneByWhere<T>(const SQLWhere: string;
  const Params: array of Variant; const RaiseExceptionIfNotFound: Boolean): T;
begin
...
class function TMVCActiveRecord.Count(const aClass
  : TMVCActiveRecordClass): int64;
var
...
begin
  ...
  if not Assigned(fPrimaryKey) then
    raise Exception.CreateFmt('Cannot delete %s without a primary key',
      [ClassName]);
  SQL := SQLGenerator.CreateDeleteSQL(fTableName, fMap, fPrimaryKeyFieldName,
    fPrimaryKeyOptions, fPrimaryKey.GetValue(Self).AsInt64);
  ExecNonQuery(SQL, false);
  OnAfterDelete;
end;

class function TMVCActiveRecord.DeleteAll(const aClass
  : TMVCActiveRecordClass): int64;
var
  lAR: TMVCActiveRecord;
begin
  lAR := aClass.Create;
  try
    Result := lAR.ExecNonQuery(lAR.SQLGenerator.CreateDeleteAllSQL
      (lAR.fTableName));
  finally
    lAR.Free;
  end;
...
begin
  ...
  try
    Result := lAR.ExecNonQuery(lAR.SQLGenerator.CreateDeleteAllSQL
      (lAR.fTableName) + lAR.SQLGenerator.CreateSQLWhereByRQL(RQL,
      lAR.GetMapping, false));
  finally
    lAR.Free;
  end;
end;

procedure TMVCActiveRecord.MapColumnToTValue(const aFieldName: string;
  const aField: TField; const aRTTIField: TRttiField);
var
...
begin
  case aField.DataType of
    ...
    ftMemo, ftWideMemo:
      begin
        if aRTTIField.FieldType.TypeKind in [tkString, tkUString, tkWideString]
        then
        begin
        ...
        begin
          ...
          begin
            raise EMVCActiveRecord.CreateFmt
              ('Property target for %s field is nil', [aFieldName]);
          end;
    ...
    raise EMVCActiveRecord.CreateFmt('Unsupported FieldType (%d) for field %s',
      [Ord(aField.DataType), aFieldName]);
  end;

end;

procedure TMVCActiveRecord.MapTValueToParam(const aValue: TValue;
  ...
  case aValue.TypeInfo.Kind of
    ...
    tkClass:
      begin
        if not aValue.IsInstanceOf(TStream) then
          raise EMVCActiveRecord.CreateFmt
            ('Unsupported reference type for param %s: %s',
...
begin
  ...
  if not fPrimaryKeyFieldName.IsEmpty then
  begin
    MapColumnToTValue(fPrimaryKeyFieldName,
      aDataSet.FieldByName(fPrimaryKeyFieldName), fPrimaryKey);
  end;
  OnAfterLoad;
end;
...
begin
  CheckAction(TMVCEntityAction.eaRetrieve);
  SQL := SQLGenerator.CreateSelectByPKSQL(fTableName, fMap,
    fPrimaryKeyFieldName, fPrimaryKeyOptions, id);
...
class function TMVCActiveRecord.Select<T>(const SQL: string;
  const Params: array of Variant; const Options: TMVCActiveRecordLoadOptions)
  : TObjectList<T>;
var
  lDataSet: TDataSet;
  lAR: TMVCActiveRecord;
...
function TMVCActiveRecord.SelectRQL(const RQL: string;
...
begin
  ...
  begin
    ...
    fSQLGenerator := TMVCSQLGeneratorRegistry.Instance.GetSQLGenerator
      (GetBackEnd).Create(GetMapping);
  end;
...
begin
  ...
  for keyvalue in fMap do
    Result := Result + sLineBreak + #9 + keyvalue.Key.Name + ' = ' +
      keyvalue.value;
end;
...
begin
  ...
  SQL := SQLGenerator.CreateUpdateSQL(fTableName, fMapNonTransientFields,
    fPrimaryKeyFieldName, fPrimaryKeyOptions);
  ExecNonQuery(SQL, false);
  OnAfterUpdate;
  OnAfterInsertOrUpdate;
...
begin
  lAR := aClass.Create;
  try
    Result := Select(aClass, lAR.GenerateSelectSQL + SQLWhere, Params,
      Connection);
  finally
    lAR.Free;
  end;
...
class function TMVCActiveRecord.Where<T>(const SQLWhere: string;
...
function TMVCEntitiesRegistry.FindEntityClassByURLSegment(const aURLSegment
  : string; out aMVCActiveRecordClass: TMVCActiveRecordClass): Boolean;
begin
  Result := fEntitiesDict.TryGetValue(aURLSegment.ToLower,
    aMVCActiveRecordClass);
end;

function TMVCEntitiesRegistry.FindProcessorByURLSegment(const aURLSegment
  : string; out aMVCEntityProcessor: IMVCEntityProcessor): Boolean;
begin
  Result := fProcessorsDict.TryGetValue(aURLSegment.ToLower,
    aMVCEntityProcessor);
end;
...
constructor MVCEntityActionsAttribute.Create(const aEntityAllowedActions
  : TMVCEntityActions);
begin
  inherited Create;
...
function TMVCSQLGeneratorRegistry.GetSQLGenerator(const aBackend: string)
  : TMVCSQLGeneratorClass;
begin
  if not fSQLGenerators.TryGetValue(aBackend, Result) then
  begin
    raise ERQLCompilerNotFound.CreateFmt('SQLGenerator not found for "%s"',
      [aBackend]);
  end;
end;
...
procedure TMVCSQLGeneratorRegistry.UnRegisterSQLGenerator
...
function TMVCSQLGenerator.TableFieldsDelimited
  (const Map: TDictionary<TRttiField, string>; const PKFieldName: string;
