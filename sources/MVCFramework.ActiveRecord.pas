...
type
  ...
  // TMVCActiveRecordBackend = (cbFirebird, cbMySQL, cbInterbase);
  ...
  public
    ...
    destructor Destroy; override;
    function CreateSelectSQLByRQL(const RQL: string;
      const Mapping: TMVCFieldsMapping): string; virtual; abstract;
...
begin
  ...
  SQL := SQLGenerator.CreateInsertSQL(fTableName, fMap, fPrimaryKeyFieldName,
    fPrimaryKeyOptions);
...
begin
  ...
  begin
    MapColumnToTValue(lItem.value, aDataSet.FieldByName(lItem.value),
      lItem.Key);
  end;
  if not fPrimaryKeyFieldName.IsEmpty then
  begin
...
begin
  ...
  SQL := SQLGenerator.CreateUpdateSQL(fTableName, fMap, fPrimaryKeyFieldName,
    fPrimaryKeyOptions);
  ExecNonQuery(SQL, false);
  OnAfterUpdate;
  OnAfterInsertOrUpdate;
