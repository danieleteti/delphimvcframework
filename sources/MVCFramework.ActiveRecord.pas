...
{$ENDIF}
  ...
  protected
    ...
    /// <summary>
    /// Optional object-level validation hook compatible with TMVCValidationEngine.
    /// Override to collect errors programmatically. Raising EMVCValidationException
    /// is left to the caller: errors added here are merged with attribute-based
    /// validators and raised together from Validate().
    /// </summary>
  ...
  public
    ...
    procedure Insert;
    /// <summary>
    /// Runs attribute-based validators plus the optional OnValidate hook.
    /// Raises EMVCValidationException if any error is produced; the field-level
    /// messages are exposed via the ValidationErrors dictionary.
...
procedure TMVCActiveRecord.OnValidate(const AErrors: PMVCValidationErrors);
begin
  // do nothing (override to collect field-level validation errors)
end;
...
begin
  ...
  try
    if fTableMap.fHasValidators then
    begin
      ...
      begin
        ...
        begin
          if not lValidator.Validate(lUnwrappedValue, Self) then
            lErrors.Add(lFieldName, lValidator.GetErrorMessage);
        end;
      end;
    end;

    // Always invoke OnValidate via virtual dispatch: the base stub is a no-op,
    // so classes that do not override incur only a cheap empty virtual call.
    OnValidate(@lErrors);

    // The exception's constructor COPIES the dictionary, so we just hand over
    // the reference and let the outer finally Clear the original.
    if lErrors.HasErrors then
      raise EMVCValidationException.Create(lErrors.Errors);
  finally
    lErrors.Clear;
  end;
end;
