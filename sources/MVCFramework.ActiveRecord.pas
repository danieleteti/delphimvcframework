...
{$ENDIF}
  ...
  TMVCActiveRecordFieldOption = (
    ...
    /// RETURNING / OUTPUT where the engine supports it; falls back to a
    /// post-execute SELECT on MySQL.
    /// </summary>
    foRefresh
  );
...
begin
  ...
  try
    ...
    // drives the RETURNING/OUTPUT read-back. fAutoGenPKIndex = -1 when none.
    ...
    begin
      ...
      begin
        ...
        begin
          lAutoGenPK := fTableMap.fPrimaryKeys[fTableMap.fAutoGenPKIndex];
          lValue := lAutoGenPK.RTTIField.GetValue(Self);
          if (lValue.Kind = tkRecord) then
            MapDataSetFieldToNullableRTTIField(lValue, lQry.FieldByName(lAutoGenPK.FieldName), lAutoGenPK.RTTIField, Self)
          else
          begin
            // Type-aware read-back. AsInteger truncates a bigint/Int64 identity
            // value to 32 bits (silent corruption of the in-memory PK). Route
            // through the shared mapper (handles ftLargeInt via AsLargeInt,
            // strings, GUIDs) exactly like the foRefresh path below and
            // FillPrimaryKey.
