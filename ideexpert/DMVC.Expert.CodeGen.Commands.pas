...
procedure TUnitUsesCommand.ExecuteInterface(Section: TStringBuilder; Model: TJSONObject);
begin
  inherited;
  Section.AppendLine('uses').AppendLine('  System.SysUtils').AppendLine('  ;').AppendLine
end;

{ TUnitDMVCLicenseCommand }

procedure TUnitDMVCLicenseCommand.ExecuteInterface(Section: TStringBuilder; Model: TJSONObject);
begin
  inherited;
  Section
      .AppendLine('// ***************************************************************************')
...
procedure TUnitProgramCommand.ExecuteImplementation(Section: TStringBuilder; Model: TJsonObject);
begin
  inherited;

end;

procedure TUnitProgramCommand.ExecuteInterface(Section: TStringBuilder; Model: TJSONObject);
begin
  inherited;
  ...
  begin
    Section
        .AppendLine('  IPPeerServer,')
        .AppendLine('  Web.FastCGIApp,')
  end;
...
procedure TUnitControllerCommand.ExecuteImplementation(Section: TStringBuilder; Model: TJsonObject);
begin
  inherited;
  Section
...
procedure TUnitControllerCommand.ExecuteInterface(Section: TStringBuilder; Model: TJSONObject);
begin
  inherited;
  Section
  ...
  begin
    Section.Append('MVCFramework.Nullables, ').Append(Model[TConfigKey.entity_unit_name] + ', ');
  end;
  if Model.B[TConfigKey.program_service_container_generate] then
  begin
    Section.Append(Model[TConfigKey.program_service_container_unit_name] + ', ');
  end;
  Section.AppendLine('MVCFramework.Serializer.Commons, System.Generics.Collections;').AppendLine.AppendLine('type')
end;

procedure TUnitControllerEntityDeclarationCommand.ExecuteImplementation(Section: TStringBuilder; Model: TJsonObject);
begin
  inherited;
  if not Model.B[TConfigKey.entity_generate] then
    Exit;
  Section
      .AppendLine('implementation')
      .AppendLine
      .AppendLine(
          'constructor '
              + Model[TConfigKey.entity_classname]
              + '.Create(ID: Integer; FirstName, LastName: String; DOB: TDate);')
      .AppendLine('begin')
...
procedure TUnitControllerEntityDeclarationCommand.ExecuteInterface(Section: TStringBuilder; Model: TJSONObject);
begin
  inherited;
  if not Model.B[TConfigKey.entity_generate] then
    Exit;
...
procedure TUnitInterfaceKeywordCommand.ExecuteImplementation(Section: TStringBuilder; Model: TJSONObject);
begin
  inherited;
  Section.AppendLine('implementation').AppendLine;
end;

procedure TUnitInterfaceKeywordCommand.ExecuteInterface(Section: TStringBuilder; Model: TJSONObject);
begin
  inherited;
...
begin
  ...
  begin
    Section
        ...
        .AppendLine(
            'procedure '
                + Model[TConfigKey.controller_classname]
                + '.OnAfterAction(Context: TWebContext; const AActionName: string);')
        .AppendLine('begin')
        ...
        .AppendLine(
            'procedure '
                + Model[TConfigKey.controller_classname]
                + '.OnBeforeAction(Context: TWebContext; const AActionName: string; var Handled: Boolean);')
        .AppendLine('begin')
  ...
  begin
    ...
    Section
        ...
        .AppendLine
        .AppendLine(
            'function ' + Model[TConfigKey.controller_classname] + '.GetReversedString(const Value: String): String;')
        .AppendLine('begin');
    if Model.B[TConfigKey.controller_actions_profiling_generate] then
    begin
      Section
    ...
    Section.AppendLine('  Result := System.StrUtils.ReverseString(Value.Trim);').AppendLine('end;')
  end;
  ...
  begin
    ...
    begin
      Section
          ...
          .AppendLine(
              'function '
                  + Model[TConfigKey.controller_classname]
                  + '.GetPeople(PeopleService: IPeopleService): IMVCResponse;')
          .AppendLine('begin');
      if Model.B[TConfigKey.controller_actions_profiling_generate] then
      begin
        Section
      ...
      Section.AppendLine('  Result := OkResponse(PeopleService.GetAll);').AppendLine('end;')
    end
    else
    begin
      Section
    ...
    Section
        ...
        .AppendLine(
            'function '
                + Model[TConfigKey.controller_classname]
                + '.CreatePerson([MVCFromBody] Person: TPerson): IMVCResponse;')
        .AppendLine('begin');
    if Model.B[TConfigKey.controller_actions_profiling_generate] then
    begin
      Section
    ...
    Section
        ...
        .AppendLine(
            'function '
                + Model[TConfigKey.controller_classname]
                + '.UpdatePerson(ID: Integer; [MVCFromBody] Person: TPerson): IMVCResponse;')
        .AppendLine('begin');
    if Model.B[TConfigKey.controller_actions_profiling_generate] then
    begin
      Section
...
procedure TUnitControllerControllerDeclarationCommand.ExecuteInterface(Section: TStringBuilder; Model: TJSONObject);
var
  lConverter: string;
begin
  inherited;
  ...
  begin
    ...
    if Model.B[TConfigKey.program_service_container_generate] then
    begin
      Section.AppendLine('    function GetPeople([MVCInject] PeopleService: IPeopleService): IMVCResponse;');
    end
    else
    begin
      Section.AppendLine('    function GetPeople: IMVCResponse;')
    end;
  ...
  Section.AppendLine('  end;').AppendLine

end;

{ TWebModuleDFMCommand }

procedure TWebModuleDFMCommand.ExecuteImplementation(Section: TStringBuilder; Model: TJsonObject);
begin
  inherited;
  Section
      .AppendLine(
          'object '
              + Model.S[TConfigKey.webmodule_classname].Substring(1)
              + ': '
              + Model[TConfigKey.webmodule_classname])
...
procedure TWebModuleDFMCommand.ExecuteInterface(Section: TStringBuilder; Model: TJSONObject);
begin
  inherited;
...
procedure TUnitWebModuleDeclarationCommand.ExecuteImplementation(Section: TStringBuilder; Model: TJsonObject);
var
...
begin
  ...
  if Model.B[TConfigKey.program_ssv_mustache] then
  begin
    Section.AppendLine('  MVCFramework.Serializer.URLEncoded,').AppendLine('  MVCFramework.View.Renderers.Mustache,');
    default_media_type := 'TMVCMediaType.TEXT_HTML';
  end;
  ...
  Section
      ...
      .AppendLine(
          '      Config[TMVCConfigKey.DefaultContentType] := dotEnv.Env(''dmvc.default.content_type'', '
              + default_media_type
              + ');')
  ...
  begin
    ...
    Section
        ...
        .AppendLine(
            '    dotEnv.Env(''firedac.connection_definitions_filename'', ''' + activerecord_con_def_filename + ''')')
        .AppendLine('  ));')
  end;

  Section.AppendLine('  // Middleware - END');
...
procedure TUnitWebModuleDeclarationCommand.ExecuteInterface(Section: TStringBuilder; Model: TJSONObject);
begin
  inherited;
...
procedure TUnitJSONRPCDeclarationCommand.ExecuteImplementation(Section: TStringBuilder; Model: TJsonObject);
begin
  inherited;
...
procedure TUnitJSONRPCDeclarationCommand.ExecuteInterface(Section: TStringBuilder; Model: TJSONObject);
begin
  inherited;
...
procedure TUnitFooterCommand.ExecuteImplementation(Section: TStringBuilder; Model: TJSONObject);
begin
  inherited;
  Section.AppendLine.AppendLine('end.');
end;

procedure TUnitFooterCommand.ExecuteInterface(Section: TStringBuilder; Model: TJsonObject);
begin
  inherited;
...
procedure TUnitMainBeginEndCommand.ExecuteImplementation(Section: TStringBuilder; Model: TJSONObject);
begin
  inherited;
  Section
      .AppendLine
  ...
  begin
    Section
        .AppendLine
        .AppendLine(
            '  TMVCSqids.SQIDS_ALPHABET := dotEnv.Env(''dmvc.sqids.alphabet'', ''' + GetScrambledAlphabet + ''');')
  ...
  Section
      ...
      .AppendLine('{$IF CompilerVersion >= 34} //SYDNEY+')
      .AppendLine(
          '    if dotEnv.Env(''dmvc.profiler.enabled'', '
              + Model.S[TConfigKey.controller_actions_profiling_generate].ToLower
              + ') then')
      .AppendLine('    begin')
  ...
  if Model.B[TConfigKey.program_ssv_templatepro] then
  begin
    Section.AppendLine('    TemplateProContextConfigure;');
  end;

  if Model.B[TConfigKey.program_ssv_webstencils] then
  begin
    Section.AppendLine('    WebStencilsProcessorConfigure;');
  end;
  ...
  Section
      .AppendLine(
          '    RunServer(dotEnv.Env(''dmvc.server.port'', ' + Model[TConfigKey.program_default_server_port] + '));')
      .AppendLine('  except')
...
procedure TUnitMainBeginEndCommand.ExecuteInterface(Section: TStringBuilder; Model: TJsonObject);
begin
  inherited;
...
begin
  ...
  begin
    ...
    Section.AppendLine('  finally').AppendLine('    LServer.Free;').AppendLine('  end;').AppendLine('end;')
  end;
  ...
  begin
    Section
        ...
        .AppendLine('begin')
        .AppendLine('  LServer := TFastCGIApplication(Application);')
        .AppendLine('  LServer.DefaultPort := APort;')
        .AppendLine('  LServer.Active := True;')
        .AppendLine('  LogI(''Listening on fastcgi://localhost:'' + APort.ToString);')
        .AppendLine('  LogI(''Application started. Press Ctrl+C to shut down.'');')
        .AppendLine('  WaitForTerminationSignal;')
        .AppendLine('  EnterInShutdownState;')
        .AppendLine('  LServer.Active := False;')
...
procedure TUnitRunServerProcBody.ExecuteInterface(Section: TStringBuilder; Model: TJsonObject);
begin
  inherited;
...
procedure TUnitControllerEntityImplementationCommand.ExecuteImplementation(Section: TStringBuilder; Model: TJSONObject);
begin
  inherited;
  if not Model.B[TConfigKey.entity_generate] then
    Exit;
...
procedure TUnitMustacheHelpersDeclarationCommand.ExecuteImplementation(Section: TStringBuilder; Model: TJsonObject);
begin
  inherited;
  Section
      .AppendLine('implementation')
...
procedure TUnitMustacheHelpersDeclarationCommand.ExecuteInterface(Section: TStringBuilder; Model: TJSONObject);
begin
  inherited;
...
procedure TUnitServicesDeclarationCommand.ExecuteImplementation(Section: TStringBuilder; Model: TJsonObject);
begin
  Section
      .AppendLine('implementation')
      .AppendLine
...
procedure TUnitServicesDeclarationCommand.ExecuteInterface(Section: TStringBuilder; Model: TJSONObject);
begin
  Section
...
procedure TUnitTemplateProHelpersDeclarationCommand.ExecuteImplementation(Section: TStringBuilder; Model: TJsonObject);
begin
  inherited;
  Section
      .AppendLine('implementation')
...
procedure TUnitTemplateProHelpersDeclarationCommand.ExecuteInterface(Section: TStringBuilder; Model: TJSONObject);
begin
  inherited;
