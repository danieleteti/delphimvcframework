...
{$IF Defined(TEST_MSSQL)}
  ...
  //
  // DMVC_MSSQL_OSAUTHENT=1 in the environment switches the connection to
  // Windows authentication, for a local SQL Server where "sa" is not usable.
  // The "guida" database must exist either way.
  ...
  public
    ...
    procedure Setup;
    [Test]
    procedure TestTrigger_InsertPKOnly_ReadsOwnIdentity;
    [Test]
    procedure TestTrigger_InsertRefreshesFields;
    [Test]
    procedure TestTrigger_UpdateRefreshesFields;
    [Test]
    procedure TestTrigger_UpdateMissingRowRaisesNotFound;
    [Test]
    procedure TestTrigger_StaleVersionRejected;
    [Test]
    procedure TestTrigger_UpdateIfChanged;
    [Test]
    procedure TestRefreshFields_NoTrigger;
    [Test]
    procedure TestTrigger_ConnectionUsableAfterFailedInsert;
    [Test]
    procedure TestTrigger_GuidAutogenPK;
    [Test]
    procedure TestTrigger_GuidAutogenPKWithRefresh;
    [Test]
    procedure TestTrigger_RefreshSeesColumnWrittenByTrigger;
    [Test]
    procedure TestTrigger_OtherEntityWorksAfterTriggerRollback;
    [Test]
    procedure TestTrigger_StaleVersionRejectedWithoutRefresh;
  end;
{$ENDIF}
...
{$IF Defined(TEST_MSSQL)}
  // The MSSQL generator leaves the driver out on purpose (Delphi Pro has none)
  FireDAC.Phys.MSSQL,
{$ENDIF}
  TestConstsU;
...
type
  { SQL Server refuses "OUTPUT inserted.col" without INTO on a table with
    enabled triggers. These entities map tables that have one (except
    TMSSQLRefNoTrig), and a trigger that inserts into its own IDENTITY table,
    so a wrong identity read-back (@@IDENTITY) shows up as a wrong ID. }
  [MVCTable('mssql_trg_plain')]
  TMSSQLTrgPlain = class(TMVCActiveRecord)
  private
    [MVCTableField('id', [foPrimaryKey, foAutoGenerated])]
    fID: Integer;
    [MVCTableField('name')]
  ...
  [MVCTable('mssql_trg_ref')]
  TMSSQLTrgRef = class(TMVCActiveRecord)
  private
    [MVCTableField('id', [foPrimaryKey, foAutoGenerated])]
    fID: Integer;
    [MVCTableField('name')]
    fName: string;
    [MVCTableField('created_at', [foReadOnly, foRefresh])]
    fCreatedAt: TDateTime;
    [MVCTableField('code', [foReadOnly, foRefresh])]
    fCode: string;
  public
    property ID: Integer read fID write fID;
    property Name: string read fName write fName;
  ...
  [MVCTable('mssql_ref_notrig')]
  TMSSQLRefNoTrig = class(TMSSQLTrgRef);

  [MVCTable('mssql_trg_ref')]
  [MVCChangeTracking]
  TMSSQLTrgRefTracked = class(TMSSQLTrgRef);

  [MVCTable('mssql_trg_guid')]
  TMSSQLTrgGuid = class(TMVCActiveRecord)
  private
    [MVCTableField('id', [foPrimaryKey, foAutoGenerated])]
    fID: TGUID;
    [MVCTableField('name')]
  ...
  [MVCTable('mssql_trg_guid')]
  TMSSQLTrgGuidRef = class(TMSSQLTrgGuid)
  private
    [MVCTableField('created_at', [foReadOnly, foRefresh])]
    fCreatedAt: TDateTime;
  public
  ...
  { upper_name is written by an AFTER trigger, which also rolls back when
    name = 'rollback'. }
  [MVCTable('mssql_trg_upper')]
  TMSSQLTrgUpper = class(TMVCActiveRecord)
  private
    [MVCTableField('id', [foPrimaryKey, foAutoGenerated])]
    fID: Integer;
    [MVCTableField('name')]
  ...
  public
    property ID: Integer read fID write fID;
    property Name: string read fName write fName;
    property UpperName: string read fUpperName;
  end;

  { Same table as TMSSQLTrgVer without the foRefresh column: the UPDATE runs as
    a plain statement, whose row count the noisy trigger must not inflate. }
  [MVCTable('mssql_trg_ver')]
  TMSSQLTrgVerPlain = class(TMVCActiveRecord)
  private
    [MVCTableField('id', [foPrimaryKey, foAutoGenerated])]
    fID: Integer;
    [MVCTableField('name')]
  ...
  [MVCTable('mssql_trg_ver')]
  TMSSQLTrgVer = class(TMVCActiveRecord)
  private
    [MVCTableField('id', [foPrimaryKey, foAutoGenerated])]
    fID: Integer;
    [MVCTableField('name')]
...
begin
  ...
  try
    ...
    if GetEnvironmentVariable('DMVC_MSSQL_OSAUTHENT') = '1' then
...
begin
  ...
  for lSQL in ['customers', 'customers2', 'customers_with_code', 'nullables_test',
    '"customers with spaces"', 'booltest', 'customers_with_guid',
    'validated_customers', 'audited_customers', 'mssql_trg_plain', 'mssql_trg_ref',
    'mssql_ref_notrig', 'mssql_trg_ver', 'mssql_trg_guid', 'mssql_trg_upper', 'mssql_trg_audit'] do
  begin
  ...
  for lSQL in [
    'create table mssql_trg_audit (id int identity(1000,1) primary key, what nvarchar(100))',
    'create table mssql_trg_plain (id int identity(1,1) primary key, name nvarchar(50))',
    'create trigger tr_mssql_trg_plain on mssql_trg_plain after insert as ' +
      'insert into mssql_trg_audit(what) select name from inserted',
    'create table mssql_trg_ref (id int identity(1,1) primary key, name nvarchar(50) unique, ' +
      'created_at datetime2 not null default sysutcdatetime(), code as (''C'' + cast(id as varchar(10))))',
    'create trigger tr_mssql_trg_ref on mssql_trg_ref after insert, update as ' +
      'insert into mssql_trg_audit(what) select name from inserted',
    'create table mssql_ref_notrig (id int identity(1,1) primary key, name nvarchar(50) unique, ' +
      'created_at datetime2 not null default sysutcdatetime(), code as (''C'' + cast(id as varchar(10))))',
    'create table mssql_trg_ver (id int identity(1,1) primary key, name nvarchar(50), ' +
      'objversion int not null, code as (''V'' + cast(id as varchar(10))))',
    // Deliberately noisy: writes a row even when the UPDATE matched none, and
    // without SET NOCOUNT, so its row count cannot pass for the UPDATE's.
    'create trigger tr_mssql_trg_ver on mssql_trg_ver after insert, update as ' +
      'insert into mssql_trg_audit(what) values (''ver touched'')',
    'create table mssql_trg_guid (id uniqueidentifier primary key default newid(), ' +
      'name nvarchar(50), created_at datetime2 not null default sysutcdatetime())',
    'create trigger tr_mssql_trg_guid on mssql_trg_guid after insert, update as ' +
      'insert into mssql_trg_audit(what) select name from inserted',
    'create table mssql_trg_upper (id int identity(1,1) primary key, name nvarchar(50), upper_name nvarchar(50))',
    'create trigger tr_mssql_trg_upper on mssql_trg_upper after insert, update as begin ' +
      'set nocount on; ' +
      'if exists (select 1 from inserted where name = ''rollback'') ' +
      'begin rollback; throw 50001, ''rollback requested'', 1; end; ' +
      'update t set upper_name = upper(i.name) from mssql_trg_upper t join inserted i on i.id = t.id; end'] do
  begin
    fConnection.ExecSQL(lSQL);
  end;
  ActiveRecordConnectionsRegistry.AddDefaultConnection(fConnection);
...
procedure TTestActiveRecordMSSQL.TestTrigger_InsertPKOnly_ReadsOwnIdentity;
var
  lP: TMSSQLTrgPlain;
  I: Integer;
begin
  // The trigger's own insert moves @@IDENTITY to 1000+: only a scope-bound
  // read-back returns the row's real id.
  for I := 1 to 3 do
  begin
    lP := TMSSQLTrgPlain.Create;
    try
      lP.Name := 'p' + I.ToString;
      lP.Insert;
      Assert.AreEqual<Integer>(fConnection.ExecSQLScalar(
        'select id from mssql_trg_plain where name = ?', [lP.Name]), lP.ID);
    finally
      lP.Free;
    end;
  end;
...
procedure TTestActiveRecordMSSQL.TestTrigger_InsertRefreshesFields;
var
  lR: TMSSQLTrgRef;
begin
  lR := TMSSQLTrgRef.Create;
  try
    lR.Name := 'r1';
    lR.Insert;
    Assert.AreEqual<Integer>(fConnection.ExecSQLScalar(
      'select id from mssql_trg_ref where name = ''r1'''), lR.ID);
    Assert.AreEqual('C' + lR.ID.ToString, lR.Code);
    Assert.IsTrue(lR.CreatedAt > EncodeDate(2020, 1, 1), 'DEFAULT column not refreshed');
  finally
    lR.Free;
  end;
end;

procedure TTestActiveRecordMSSQL.TestTrigger_UpdateRefreshesFields;
var
  lR: TMSSQLTrgRef;
  lID: Integer;
begin
  ...
  lR := TMVCActiveRecord.GetByPK<TMSSQLTrgRef>(lID);
  try
    lR.Name := 'r1-upd';
    lR.Update;
    Assert.AreEqual('r1-upd', string(fConnection.ExecSQLScalar(
      'select name from mssql_trg_ref where id = ?', [lID])));
    Assert.AreEqual('C' + lID.ToString, lR.Code);
  finally
    lR.Free;
  end;
end;

procedure TTestActiveRecordMSSQL.TestTrigger_UpdateMissingRowRaisesNotFound;
var
  lR: TMSSQLTrgRef;
begin
  lR := TMSSQLTrgRef.Create;
...
procedure TTestActiveRecordMSSQL.TestTrigger_StaleVersionRejected;
var
  lA, lB: TMSSQLTrgVer;
begin
  lA := TMSSQLTrgVer.Create;
  try
    lA.Name := 'v1';
    lA.Insert;
    Assert.AreEqual('V' + lA.ID.ToString, lA.Code);
    lB := TMVCActiveRecord.GetByPK<TMSSQLTrgVer>(lA.ID);
    try
      lA.Name := 'v1-first';
      lA.Update;
      lB.Name := 'v1-stale';
      Assert.WillRaise(
        ...
        begin
          lB.Update;
        end, EMVCActiveRecordVersionedItemNotFound);
      Assert.AreEqual('v1-first', string(fConnection.ExecSQLScalar(
        'select name from mssql_trg_ver where id = ?', [lA.ID])));
    finally
      lB.Free;
    end;
  finally
...
procedure TTestActiveRecordMSSQL.TestTrigger_StaleVersionRejectedWithoutRefresh;
var
  lA, lB: TMSSQLTrgVerPlain;
begin
  lA := TMSSQLTrgVerPlain.Create;
  try
    lA.Name := 'p1';
    lA.Insert;
    lB := TMVCActiveRecord.GetByPK<TMSSQLTrgVerPlain>(lA.ID);
    try
      lA.Name := 'p1-first';
      lA.Update;
      lB.Name := 'p1-stale';
      ...
      Assert.AreEqual('p1-first', string(fConnection.ExecSQLScalar(
        'select name from mssql_trg_ver where id = ?', [lA.ID])));
    finally
      lB.Free;
    end;
...
procedure TTestActiveRecordMSSQL.TestTrigger_UpdateIfChanged;
var
  lR: TMSSQLTrgRefTracked;
  lID: Integer;
begin
  lR := TMSSQLTrgRefTracked.Create;
  try
    lR.Name := 'r1';
    lR.Insert;
    lID := lR.ID;
  ...
  lR := TMVCActiveRecord.GetByPK<TMSSQLTrgRefTracked>(lID);
  try
    lR.Name := 'r1-tracked';
    lR.UpdateIfChanged;
    Assert.AreEqual('r1-tracked', string(fConnection.ExecSQLScalar(
      'select name from mssql_trg_ref where id = ?', [lID])));
    Assert.AreEqual('C' + lID.ToString, lR.Code);
  finally
    lR.Free;
...
procedure TTestActiveRecordMSSQL.TestRefreshFields_NoTrigger;
var
  lR: TMSSQLRefNoTrig;
begin
  lR := TMSSQLRefNoTrig.Create;
  try
    lR.Name := 'n1';
    lR.Insert;
    Assert.AreEqual<Integer>(fConnection.ExecSQLScalar(
      'select id from mssql_ref_notrig where name = ''n1'''), lR.ID);
    Assert.AreEqual('C' + lR.ID.ToString, lR.Code);
    lR.Name := 'n1-upd';
    lR.Update;
    Assert.AreEqual('n1-upd', string(fConnection.ExecSQLScalar(
      'select name from mssql_ref_notrig where id = ?', [lR.ID])));
  finally
    lR.Free;
  end;
end;

procedure TTestActiveRecordMSSQL.TestTrigger_ConnectionUsableAfterFailedInsert;
var
  lR: TMSSQLTrgRef;
begin
  // A batch that fails midway leaves #dmvc_output behind in the session; the
  // next statement on the same connection must drop it and go on.
  lR := TMSSQLTrgRef.Create;
  try
    lR.Name := 'dup';
    lR.Insert;
  ...
  try
    ...
    lR.Insert;
    Assert.AreEqual<Integer>(fConnection.ExecSQLScalar(
      'select id from mssql_trg_ref where name = ''after'''), lR.ID);
    Assert.AreEqual('C' + lR.ID.ToString, lR.Code);
  finally
    lR.Free;
  end;
...
procedure TTestActiveRecordMSSQL.TestTrigger_GuidAutogenPK;
var
  lG: TMSSQLTrgGuid;
begin
  // DEFAULT NEWID() is not an IDENTITY: SCOPE_IDENTITY() is NULL here.
  lG := TMSSQLTrgGuid.Create;
  try
    lG.Name := 'g1';
    lG.Insert;
    Assert.AreEqual(string(fConnection.ExecSQLScalar(
      'select cast(id as varchar(36)) from mssql_trg_guid where name = ''g1''')).ToUpper,
      GUIDToString(lG.ID).Substring(1, 36));
  finally
    lG.Free;
  end;
end;

procedure TTestActiveRecordMSSQL.TestTrigger_GuidAutogenPKWithRefresh;
var
  lG: TMSSQLTrgGuidRef;
begin
  lG := TMSSQLTrgGuidRef.Create;
  try
    lG.Name := 'g2';
    lG.Insert;
    Assert.AreEqual(string(fConnection.ExecSQLScalar(
      'select cast(id as varchar(36)) from mssql_trg_guid where name = ''g2''')).ToUpper,
      GUIDToString(lG.ID).Substring(1, 36));
    Assert.IsTrue(lG.CreatedAt > EncodeDate(2020, 1, 1), 'DEFAULT column not refreshed');
  finally
    lG.Free;
  end;
end;

procedure TTestActiveRecordMSSQL.TestTrigger_RefreshSeesColumnWrittenByTrigger;
var
  lU: TMSSQLTrgUpper;
begin
  // OUTPUT reports the row as it was before AFTER triggers ran; foRefresh must
  // bring back what the trigger wrote.
  lU := TMSSQLTrgUpper.Create;
  try
    lU.Name := 'abc';
    lU.Insert;
    Assert.AreEqual('ABC', lU.UpperName);
    lU.Name := 'def';
    lU.Update;
    Assert.AreEqual('DEF', lU.UpperName);
  finally
    lU.Free;
  end;
end;

procedure TTestActiveRecordMSSQL.TestTrigger_OtherEntityWorksAfterTriggerRollback;
var
  lU: TMSSQLTrgUpper;
  lR: TMSSQLTrgRef;
begin
  // A trigger ROLLBACK aborts the whole batch: nothing it left in the session
  // may break the next save of a different entity on the same connection.
  lU := TMSSQLTrgUpper.Create;
  try
    lU.Name := 'rollback';
    Assert.WillRaiseAny(
