...
uses
  ...
  MVCFramework.Validation, MVCFramework.Validators, MVCFramework.ValidationEngine,
...
const
  SQLs_SQLITE: array [0 .. 8] of string =
    ...
    'CREATE TABLE validated_customers (id INTEGER PRIMARY KEY AUTOINCREMENT, description varchar(10), email varchar(200), rating INTEGER)',
    'CREATE TABLE audited_customers (id INTEGER PRIMARY KEY AUTOINCREMENT, description varchar(200), created_at TIMESTAMP, updated_at TIMESTAMP, created_by varchar(100), updated_by varchar(100))'
    );

  SQLs_FIREBIRD: array [0 .. 8] of string =
    ...
    'CREATE TABLE validated_customers (id integer generated by default as identity primary key, description varchar(10), email varchar(200), rating INTEGER)',
    'CREATE TABLE audited_customers (id integer generated by default as identity primary key, description varchar(200), created_at TIMESTAMP, updated_at TIMESTAMP, created_by varchar(100), updated_by varchar(100))'
    );

  { Oracle: VARCHAR2 rather than VARCHAR, NUMBER instead of INTEGER, and no TIME
    type at all, so the time-only column becomes a TIMESTAMP. BOOLEAN is native
    from Oracle 23 on, which is the version the container in ActiveRecordTestsU
    runs. Used only when TEST_ORACLE is defined. }
  SQLs_ORACLE: array [0 .. 8] of string =
    ('CREATE TABLE customers (id number(10) generated by default as identity primary key, code varchar2(20), description varchar2(200), city varchar2(200), note varchar2(200), rating number(10), creation_time TIMESTAMP, creation_date date)',
    'CREATE TABLE customers2 (id number(10) generated by default as identity primary key, code varchar2(20), description varchar2(200), city varchar2(200), note varchar2(200), rating number(10), creation_time TIMESTAMP, creation_date date)',
    'CREATE TABLE customers_with_code (code varchar2(20) not null, description varchar2(200), city varchar2(200), note varchar2(200), rating number(10), PRIMARY KEY(code))',
    'CREATE TABLE nullables_test(f_int2 number(5), f_int8 number(19), f_int4 number(10), f_string varchar2(200), f_bool BOOLEAN, '
    + 'f_date TIMESTAMP, f_time TIMESTAMP, f_datetime TIMESTAMP, f_float4 binary_float, f_float8 binary_double, '
    + 'f_currency number(18,4), f_blob BLOB)',
    'CREATE TABLE "customers with spaces" ( ' +
    '"id with spaces" number(19) generated by default as identity primary key, ' +
    '"code with spaces" varchar2(20), ' +
    '"description with spaces" varchar2(200), ' +
    '"city with spaces" varchar2(200), ' +
    '"note with spaces" CLOB, ' +
    '"rating with spaces" number(10), ' +
    '"creation_time with spaces" TIMESTAMP, ' +
    '"creation_date with spaces" date' +
    ')',
    'CREATE TABLE booltest(id number(10) generated by default as identity primary key, boolvalue BOOLEAN)',
    'CREATE TABLE customers_with_guid (idguid varchar2(38) NOT NULL, OtherGUID varchar2(38) not null, code varchar2(20), description varchar2(200), city varchar2(200), note varchar2(200), RATING number(10), CONSTRAINT CUSTOMERS_WITH_GUID_PK PRIMARY KEY (idguid))',
    'CREATE TABLE validated_customers (id number(10) generated by default as identity primary key, description varchar2(10), email varchar2(200), rating number(10))',
    'CREATE TABLE audited_customers (id number(10) generated by default as identity primary key, description varchar2(200), created_at TIMESTAMP, updated_at TIMESTAMP, created_by varchar2(100), updated_by varchar2(100))'
    );

  { SQL Server: IDENTITY(1,1), BIT for booleans, UNIQUEIDENTIFIER for GUIDs and
    VARCHAR(MAX) where the others use an unbounded text type. Used only when
    TEST_MSSQL is defined. }
  SQLs_MSSQL: array [0 .. 8] of string =
    ('CREATE TABLE customers (id int identity(1,1) primary key, code varchar (20), description varchar (200), city varchar (200), note varchar(200), rating INTEGER, creation_time TIME, creation_date date)',
    'CREATE TABLE customers2 (id int identity(1,1) primary key, code varchar (20), description varchar (200), city varchar (200), note varchar(200), rating INTEGER, creation_time TIME, creation_date date)',
    'CREATE TABLE customers_with_code (code varchar (20) not null, description varchar (200), city varchar (200), note varchar(200), rating INTEGER, PRIMARY KEY(code))',
    'CREATE TABLE nullables_test(f_int2 smallint, f_int8 bigint, f_int4 int, f_string varchar(200), f_bool BIT, '
    + 'f_date DATETIME2, f_time TIME, f_datetime DATETIME2, f_float4 real, f_float8 float, '
    + 'f_currency numeric(18,4), f_blob VARBINARY(MAX))',
    'CREATE TABLE "customers with spaces" ( ' +
    '"id with spaces" bigint identity(1,1) primary key, ' +
    '"code with spaces" varchar(20) NULL, ' +
    '"description with spaces" varchar(200) NULL, ' +
    '"city with spaces" varchar(200) NULL, ' +
    '"note with spaces" varchar(MAX) NULL, ' +
    '"rating with spaces" int NULL, ' +
    '"creation_time with spaces" time NULL, ' +
    '"creation_date with spaces" date NULL' +
    ')',
    'CREATE TABLE booltest(id int identity(1,1) primary key, boolvalue BIT)',
    'CREATE TABLE customers_with_guid (idguid UNIQUEIDENTIFIER NOT NULL, OtherGUID UNIQUEIDENTIFIER not null, code VARCHAR(20), description VARCHAR(200), city VARCHAR(200), note varchar(200), RATING int, CONSTRAINT CUSTOMERS_WITH_GUID_PK PRIMARY KEY (idguid))',
    'CREATE TABLE validated_customers (id int identity(1,1) primary key, description varchar(10), email varchar(200), rating INTEGER)',
    'CREATE TABLE audited_customers (id int identity(1,1) primary key, description varchar(200), created_at DATETIME2, updated_at DATETIME2, created_by varchar(100), updated_by varchar(100))'
    );

  { MariaDB and MySQL: AUTO_INCREMENT instead of GENERATED BY DEFAULT AS
    IDENTITY, no UUID type (a CHAR(38) holds the braced form FireDAC writes),
    and DATETIME rather than TIMESTAMP - the first TIMESTAMP column of a table
    would otherwise get the implicit DEFAULT CURRENT_TIMESTAMP ON UPDATE
    CURRENT_TIMESTAMP, which quietly rewrites values the tests just wrote. }
  SQLs_MARIADB: array [0 .. 8] of string =
    ('CREATE TABLE customers (id integer not null auto_increment primary key, code varchar (20), description varchar (200), city varchar (200), note varchar(200), rating INTEGER, creation_time TIME, creation_date date)',
    'CREATE TABLE customers2 (id integer not null auto_increment primary key, code varchar (20), description varchar (200), city varchar (200), note varchar(200), rating INTEGER, creation_time TIME, creation_date date)',
    'CREATE TABLE customers_with_code (code varchar (20) not null, description varchar (200), city varchar (200), note varchar(200), rating INTEGER, PRIMARY KEY(code))',
    'CREATE TABLE nullables_test(f_int2 smallint, f_int8 bigint, f_int4 integer, f_string varchar(200), f_bool BOOLEAN, '
    + 'f_date DATETIME, f_time TIME, f_datetime DATETIME, f_float4 float, f_float8 double precision, '
    + 'f_currency numeric(18,4), f_blob BLOB)',
    'CREATE TABLE `customers with spaces` ( ' +
    '`id with spaces` bigint not null auto_increment primary key, ' +
    '`code with spaces` varchar(20) NULL, ' +
    '`description with spaces` varchar(200) NULL, ' +
    '`city with spaces` varchar(200) NULL, ' +
    '`note with spaces` text NULL, ' +
    '`rating with spaces` integer NULL, ' +
    '`creation_time with spaces` time NULL, ' +
    '`creation_date with spaces` date NULL' +
    ')',
    'CREATE TABLE booltest(id integer not null auto_increment primary key, boolvalue boolean)',
    'CREATE TABLE customers_with_guid (idguid CHAR(38) NOT NULL, OtherGUID CHAR(38) not null, code VARCHAR(20), description VARCHAR(200), city VARCHAR(200), note varchar(200), RATING integer, CONSTRAINT CUSTOMERS_WITH_GUID_PK PRIMARY KEY (idguid))',
    'CREATE TABLE validated_customers (id integer not null auto_increment primary key, description varchar(10), email varchar(200), rating INTEGER)',
    { DATETIME(3), not plain DATETIME: MySQL and MariaDB default to zero decimal
      places and truncate to whole seconds, so the value read back no longer
      matched the one the framework had just written, and the audit test that
      checks created_at does not move on Update failed on a truncation rather
      than on the behaviour it means to verify. }
    'CREATE TABLE audited_customers (id integer not null auto_increment primary key, description varchar(200), created_at DATETIME(3), updated_at DATETIME(3), created_by varchar(100), updated_by varchar(100))'
    );

  SQLs_POSTGRESQL: array [0 .. 8] of string =
    ('CREATE TABLE customers (id integer generated by default as identity primary key, code varchar (20), description varchar (200), city varchar (200), note varchar(200), rating INTEGER, creation_time TIME, creation_date date)',
    'CREATE TABLE customers2 (id integer generated by default as identity primary key,code varchar (20), description varchar (200), city varchar (200), note varchar(200), rating INTEGER, creation_time TIME, creation_date date)',
    'CREATE TABLE customers_with_code (code varchar (20) not null, description varchar (200), city varchar (200), note varchar(200), rating INTEGER, PRIMARY KEY(code))',
    'CREATE TABLE nullables_test(f_int2 smallint, f_int8 bigint, f_int4 integer, f_string varchar(200), f_bool BOOLEAN, '
...
type
  ...
  { Both spellings: FireDAC reports MariaDB through its MySQL RDBMS kind on some
    versions and as its own kind on others, and the test compares the string the
  ...
  // ==== Entities used by ActiveRecord validation/audit tests =================

  [MVCTable('validated_customers')]
  TValidatedCustomer = class(TMVCActiveRecord)
  private
    [MVCTableField('id', [foPrimaryKey, foAutoGenerated])]
    [MVCRequired]   // intentionally present: must be SKIPPED on INSERT
    fID: NullableInt32;
    [MVCTableField('description')]
    [MVCRequired]
    [MVCMaxLength(10)]
  ...
  // Same table, same fields, but with OnValidate override producing
  // a programmatic error. Used to test the OnValidate hook.
  [MVCTable('validated_customers')]
  TValidatedCustomerWithOnValidate = class(TMVCActiveRecord)
  private
    [MVCTableField('id', [foPrimaryKey, foAutoGenerated])]
    fID: NullableInt32;
    [MVCTableField('description')]
  ...
  // Intentionally broken: MVCAuditCreatedAt on a NullableTTime field.
  // Creating an instance must raise EMVCActiveRecord from InitTableInfo.
  [MVCTable('audited_customers')]
  TBadAuditTimeType = class(TMVCActiveRecord)
  private
    [MVCTableField('id', [foPrimaryKey, foAutoGenerated])]
    fID: NullableInt32;
    [MVCTableField('created_at')]
    [MVCAuditCreatedAt]
    fCreatedAt: NullableTTime; // not TDateTime / NullableTDateTime / NullableTDate
  public
    property ID: NullableInt32 read fID write fID;
    property CreatedAt: NullableTTime read fCreatedAt write fCreatedAt;
  end;

  // Intentionally broken: MVCAuditCreatedBy on an Integer field.
  [MVCTable('audited_customers')]
  TBadAuditByType = class(TMVCActiveRecord)
  private
    [MVCTableField('id', [foPrimaryKey, foAutoGenerated])]
    fID: NullableInt32;
    [MVCTableField('created_by')]
    [MVCAuditCreatedBy]
    fCreatedBy: Integer; // not string / NullableString
  public
    property ID: NullableInt32 read fID write fID;
  ...
  [MVCTable('audited_customers')]
  TAuditedCustomer = class(TMVCActiveRecord)
  private
    [MVCTableField('id', [foPrimaryKey, foAutoGenerated])]
    fID: NullableInt32;
    [MVCTableField('description')]
