...
const
  SQLs_SQLITE: array [0 .. 3] of string =
  ...
  SQLs_FIREBIRD: array [0 .. 3] of string =
...
type
  ...
  protected
    ...
    procedure MapObjectToParams(const Params: TFDParams;
  ...
  public
    ...
    property ValueAsInteger: TValue read FValueAsInteger
  ...
  public
    ...
    property PropStringList: TStringList read fPropStringList
      write SetPropStringList;
  ...
  public
    ...
    property PropStringList: TStringList read fPropStringList
      write SetPropStringList;
    property PropStringArray: TArray<string> read FPropStringArray
      write SetPropStringArray;
    [MVCSerializeAsString]
    property PropStreamAsString: TStringStream read FPropStreamAsString
      write SetPropStreamAsString;
    property PropStreamAsBASE64: TStringStream read FPropStreamAsBASE64
      write SetPropStreamAsBASE64;
  end;
...
begin
  ...
  Result := Result and (Self.fPropJSONObject.ToJSON()
    = Obj.PropJSONObject.ToJSON());
end;
...
begin
  Result := true;
  Result := Result and (TStringStream(Self.PropStream)
    .DataString = TStringStream(Obj.PropStream).DataString);
  Result := Result and (TStringStream(Self.Prop8Stream)
    .DataString = TStringStream(Obj.Prop8Stream).DataString);
...
constructor TResponseWrapper<T>.Create(TotalItems: Integer;
  aItems: TObjectList<T>);
begin
  inherited Create;
...
begin
  ...
  begin
    ...
    Result := Result and
      (Self.ValueAsString.AsString = lOther.ValueAsString.AsString);
    Result := Result and
      (Self.ValueAsInteger.AsInteger = lOther.ValueAsInteger.AsInteger);
...
begin
  ...
  Result := Result and (string.Join(',', lOther.PropStringArray)
    = string.Join(',', Self.PropStringArray));
  Result := Result and
    (lOther.PropStreamAsString.DataString = PropStreamAsString.DataString);
  Result := Result and
    (lOther.PropStreamAsBASE64.DataString = PropStreamAsBASE64.DataString);
end;
