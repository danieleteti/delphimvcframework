...
procedure Logo;
begin
  Box('DMVCFramework TEST SERVER (HTTP.sys)',
  [
      'PLATFORM'.PadRight(25) + gPLATFORM,
      'DMVCFRAMEWORK VERSION'.PadRight(25) + DMVCFRAMEWORK_VERSION,
      'SERVER TYPE'.PadRight(25) + 'HTTP.sys (kernel-mode IOCP)',
...
begin
  Logo;
  WriteLn(Format('Starting HTTP.sys Server on port %d', [APort]));
