...
procedure Logo;
begin
  Box('DMVCFramework TEST SERVER (Indy Direct)',
  [
      'PLATFORM'.PadRight(25) + gPLATFORM,
      'DMVCFRAMEWORK VERSION'.PadRight(25) + DMVCFRAMEWORK_VERSION,
      'SERVER TYPE'.PadRight(25) + 'TMVCIndyServer (no WebBroker)',
...
begin
  Logo;
  WriteLn(Format('Starting Indy Direct HTTP Server on port %d', [APort]));
