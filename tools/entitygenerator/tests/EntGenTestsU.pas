...
type
  ...
  strict private
    ...
    procedure CreateTableWithNaturalPK;
  ...
  public
    ...
    [Test] procedure TestAutoGeneratedPKIsNotDeserializable;
    [Test] procedure TestNaturalPKStaysDeserializable;
...
procedure TTestEntGenFieldOptions_PG.CreateTableWithNaturalPK;
begin
  // No BIGSERIAL: the caller supplies the key, so it must stay deserializable.
  FConn.ExecSQL(
    'CREATE TABLE ' + TEST_TABLE_PLAIN + ' (' +
...
procedure TTestEntGenFieldOptions_PG.TestAutoGeneratedPKIsNotDeserializable;
var
  lCfg: TEntGenConfig;
  lSrc: string;
begin
  // An auto-generated PK is decided by the database, never by the caller. Left
  // writable and deserializable, the id travels in from the request body: on an
  // update that is the caller choosing which row to overwrite. TMVCActiveRecord
  // protects its own base class the same way (ActiveRecord.pas, [MVCDoNotDeserialize]
  // on the inherited PK), so a generated entity must not be weaker than the
  // framework's own.
  CreateTableWithComputedColumn;
  lCfg := MakeBaseConfig;
  lCfg.Tables := [TEST_TABLE_REFRESHABLE];
  ...
  Assert.Contains(lSrc, '[MVCDoNotDeserialize]' + sLineBreak + '    property ID',
    'the auto-generated PK property is deserializable: a request body can set it');
end;

procedure TTestEntGenFieldOptions_PG.TestNaturalPKStaysDeserializable;
var
  lCfg: TEntGenConfig;
  lSrc: string;
begin
  // The other half of the rule: a key the client has to provide must keep
  // arriving from the body. Guards an IsAutoIncField that grows too generous.
  CreateTableWithNaturalPK;
  lCfg := MakeBaseConfig;
  lCfg.Tables := [TEST_TABLE_PLAIN];

  lSrc := GenerateSource(lCfg);

  Assert.Contains(lSrc, '[MVCTableField(''code'', [foPrimaryKey])]', 'PK line missing or wrong');
  Assert.DoesNotContain(lSrc, '[MVCDoNotDeserialize]',
    'a natural PK must stay deserializable: nothing else can supply it');
end;
